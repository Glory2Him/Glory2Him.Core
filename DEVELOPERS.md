...
- `.claude/settings.json` turns Claude Code's own attribution off, points git at
  ...
  review whose text carries attribution, whether it is written through the
  GitHub connector or with `gh`. The GitHub connector appends its own "Generated by"
  footer server-side when it creates a PR, issue or comment — after that check —
  so a post-create hook tells the session to edit it away at once.
- `.githooks/` refuses the commit (`pre-commit`, `pre-merge-commit`,
  `commit-msg`) and the push (`pre-push`, which also catches rewritten history
  and anything committed through a path that runs no hook). A clone outside Claude Code
...
All three share one rule set, `.githooks/identity-guard.sh`: the session hook
only reads the command or payload, and every identity and text it finds is
judged by that script. `.githooks/tests/identity-guard.test.sh` drives every
layer end to end in scratch repositories, and `rejectAiAttribution` runs it
first.
