...
// The Glory 2 Him home mockup (D:\Temp\G2H\samples\Home.html): verse-of-the-day strip, a
...
const SmallHero = ({ post }: { post: SamplePost }) => (
    <PostHeroCard
        title={post.title}
        href="/SamplePages/Post/Post-Single-Magazine"
...
export const HomeDefaultSample = () => {
    useDocumentTitle('Home Default — Sample — Glory 2 Him');

    return (
        <SampleShell title="Home Default" sourceFile="Home.html">
            <VerseOfTheDay
                verse={verseOfTheDay}
                href="/SamplePages/Post/Post-Single-Magazine" />
                        ...
                        <div className="col-lg-9">
                            ...
                            <div className="row gy-4">
                                {latest.map((post, index) => (
                                    <div className="col-sm-6" key={`${post.slug}-${index}`}>
                                        <ArticleCard
                                            title={post.title}
                                            href="/SamplePages/Post/Post-Single-Magazine"
                        ...
                        <div className="col-lg-3">
                            ...
                            <div className="d-flex flex-wrap mb-4" style={{ gap: '8px' }}>
                                {categories.map(([label, buttonCss]) => (
                                    <Link to="/Categories" className={`btn btn-sm ${buttonCss} mb-0`} key={label}>
                            ...
                            <div className="d-flex flex-wrap" style={{ gap: '8px' }}>
                                {popularTags.map(([label, buttonCss]) => (
                                    <Link
                                        to={`/Tag?name=${encodeURIComponent(label)}`}
                            ...
                            <div className="d-flex flex-wrap" style={{ gap: '8px' }}>
                                {popularReferences.map((reference) => (
                                    <Link
                                        to={`/Search-Result?q=${encodeURIComponent(reference)}`}
