...
describe('toReviewPanelItems', () => {
    describe('a review', () => {
        ...
        /// MATCHED ON THE ID, not taken off the front of the list. Every other positive
        /// assertion in this file happens to resolve the FIRST entry of the fixture, so an
        /// implementation reading `collection[0]` would satisfy them all - this one aims past
        /// row zero, which is the only way that mutation shows up.
        it('should resolve a reviewer who is not the first entry of the collection', () => {
            // when
            const item = toApprovalReviewItem(review('user-john'), resolvedNames);

            // then
            expect(item.reviewerDisplayName).toBe('John');
            expect(item.reviewerUserName).toBe('john.b');
        });
    ...
    describe('a request', () => {
        ...
        /// The same guard as on the review side, and it matters more here: a request row that
        /// took row zero's username would label an outstanding invitation with somebody else's
        /// account - the exact confusion the username was added to remove.
        it('should resolve a request addressed past the first entry', () => {
            // given
            const johnsRequest: ApprovalReviewRequest = {
                ...request,
                requestedUserId: 'user-john',
                requestedUserDisplayName: 'John'
            };

            // when
            const item = toRequestedReviewerItem(johnsRequest, resolvedNames);

            // then
            expect(item.userName).toBe('john.b');
        });
