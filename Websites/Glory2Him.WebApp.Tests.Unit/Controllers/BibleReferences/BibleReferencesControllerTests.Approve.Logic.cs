...
namespace Glory2Him.WebApp.Tests.Unit.Controllers.BibleReferences
{
    public partial class BibleReferencesControllerTests
    {
        ...
        public async Task ShouldReturnOkOnApproveAsync()
        {
            ...
            BibleReference storageBibleReference = inputBibleReference.DeepClone();
            ...
            bibleReferenceServiceMock
                .Setup(service => service.TransitionBibleReferenceApprovalAsync(inputBibleReference, It.IsAny<CancellationToken>()))
                    .ReturnsAsync(storageBibleReference);

            // when
            ActionResult<BibleReference> actualActionResult = await bibleReferencesController.TransitionBibleReferenceApprovalAsync(randomBibleReference, TestContext.Current.CancellationToken);

            // then
            actualActionResult.ShouldBeEquivalentTo(expectedActionResult);

            bibleReferenceServiceMock
               .Verify(service => service.TransitionBibleReferenceApprovalAsync(inputBibleReference, It.IsAny<CancellationToken>()),
                   Times.Once);
