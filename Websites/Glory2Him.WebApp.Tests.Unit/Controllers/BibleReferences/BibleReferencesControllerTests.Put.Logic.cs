    ...
    public partial class BibleReferencesControllerTests
    {
        ...
        public async Task ShouldReturnOkOnPutAsync()
        {
            ...
            bibleReferenceServiceMock
                .Setup(service => service.ModifyBibleReferenceAsync(inputBibleReference, It.IsAny<CancellationToken>()))
                    .ReturnsAsync(storageBibleReference);

            // when
            ActionResult<BibleReference> actualActionResult = await bibleReferencesController.PutBibleReferenceAsync(randomBibleReference, TestContext.Current.CancellationToken);

            // then
            actualActionResult.ShouldBeEquivalentTo(expectedActionResult);

            bibleReferenceServiceMock
               .Verify(service => service.ModifyBibleReferenceAsync(inputBibleReference, It.IsAny<CancellationToken>()),
                   Times.Once);
