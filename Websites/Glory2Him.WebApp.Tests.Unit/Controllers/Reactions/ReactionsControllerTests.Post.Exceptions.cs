    ...
    public partial class ReactionsControllerTests
    {
        ...
        public async Task ShouldReturnBadRequestOnPostIfValidationErrorOccurredAsync(Xeption validationException)
        {
            ...
            this.reactionServiceMock.Setup(service =>
                service.AddReactionAsync(It.IsAny<Reaction>(), It.IsAny<CancellationToken>()))
                    .ThrowsAsync(validationException);

            // when
            ActionResult<Reaction> actualActionResult =
                await this.reactionsController.PostReactionAsync(someReaction, TestContext.Current.CancellationToken);
        ...
        [Fact]
        public async Task ShouldReturnConflictOnPostIfAlreadyExistsReactionErrorOccurredAsync()
        {
            // given
            Reaction someReaction = CreateRandomReaction();
