...
namespace Glory2Him.WebApp.Tests.Unit.Controllers.Reactions
{
    public partial class ReactionsControllerTests
    {
        ...
        public async Task ShouldReturnOkOnPutAsync()
        {
            ...
            reactionServiceMock
                .Setup(service => service.ModifyReactionAsync(inputReaction, It.IsAny<CancellationToken>()))
                    .ReturnsAsync(storageReaction);

            // when
            ActionResult<Reaction> actualActionResult = await reactionsController.PutReactionAsync(randomReaction, TestContext.Current.CancellationToken);

            // then
            actualActionResult.ShouldBeEquivalentTo(expectedActionResult);

            reactionServiceMock
               .Verify(service => service.ModifyReactionAsync(inputReaction, It.IsAny<CancellationToken>()),
                   Times.Once);
