...
namespace Glory2Him.WebApp.Tests.Unit.Controllers.Reactions
{
    public partial class ReactionsControllerTests
    {
        ...
        public async Task ShouldRemoveRecordOnDeleteByIdsAsync()
        {
            ...
            reactionServiceMock
                .Setup(service => service.RemoveReactionByIdAsync(
                    It.IsAny<Guid>(),
                    inputDeletionReason,
            ...
            ActionResult<Reaction> actualActionResult =
                await reactionsController.DeleteReactionByIdAsync(randomReaction.Id, inputDeletionReason, TestContext.Current.CancellationToken);

            // then
            actualActionResult.ShouldBeEquivalentTo(expectedActionResult);

            reactionServiceMock
                .Verify(service => service.RemoveReactionByIdAsync(
                    It.IsAny<Guid>(),
                    inputDeletionReason,
                    It.IsAny<CancellationToken>()),
                        Times.Once);
