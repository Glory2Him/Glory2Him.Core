    ...
    public partial class ReactionsControllerTests
    {
            ...
            Xeption validationException)
        {
            ...
            this.reactionServiceMock.Setup(service =>
                service.HardRemoveReactionByIdAsync(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
                    .ThrowsAsync(validationException);

            // when
            ActionResult<Reaction> actualActionResult =
                await this.reactionsController.HardDeleteReactionByIdAsync(someId, TestContext.Current.CancellationToken);
        ...
        [Fact]
        public async Task ShouldReturnConflictOnHardDeleteIfAlreadyExistsReactionErrorOccurredAsync()
        {
            // given
            Guid someId = Guid.NewGuid();
            var someInnerException = new Exception();
