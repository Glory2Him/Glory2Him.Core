...
namespace Glory2Him.WebApp.Tests.Unit.Controllers.Links
{
    public partial class LinksControllerTests
    {
        ...
        public async Task ShouldReturnOkOnPutAsync()
        {
            ...
            Link storageLink = inputLink.DeepClone();
            ...
            linkProcessingServiceMock
                .Setup(service => service.ModifyLinkAsync(inputLink, It.IsAny<CancellationToken>()))
                    .ReturnsAsync(storageLink);

            // when
            ActionResult<Link> actualActionResult = await linksController.PutLinkAsync(randomLink, TestContext.Current.CancellationToken);

            // then
            actualActionResult.ShouldBeEquivalentTo(expectedActionResult);

            linkProcessingServiceMock
               .Verify(service => service.ModifyLinkAsync(inputLink, It.IsAny<CancellationToken>()),
                   Times.Once);
