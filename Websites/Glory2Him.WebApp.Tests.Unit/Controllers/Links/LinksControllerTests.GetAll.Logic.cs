...
namespace Glory2Him.WebApp.Tests.Unit.Controllers.Links
{
    public partial class LinksControllerTests
    {
        ...
        public async Task ShouldReturnRecordsOnGetAsync()
        {
            ...
            IQueryable<Link> storageLinks = randomLinks.DeepClone();
            IQueryable<Link> expectedLink = storageLinks.DeepClone();

            var expectedObjectResult =
                new OkObjectResult(expectedLink);

            var expectedActionResult =
                new ActionResult<IQueryable<Link>>(expectedObjectResult);

            linkProcessingServiceMock
                .Setup(service => service.RetrieveAllLinksAsync(It.IsAny<CancellationToken>()))
                    .ReturnsAsync(storageLinks);

            // when
            ActionResult<IQueryable<Link>> actualActionResult =
                await linksController.Get(TestContext.Current.CancellationToken);

            // then
            actualActionResult.ShouldBeEquivalentTo(expectedActionResult);

            linkProcessingServiceMock
               .Verify(service => service.RetrieveAllLinksAsync(It.IsAny<CancellationToken>()),
                   Times.Once);
