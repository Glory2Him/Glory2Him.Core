...
namespace Glory2Him.WebApp.Tests.Unit.Controllers.Links
{
    ...
    {
        /// <summary>
        /// Pins WHICH member the public route calls. RetrieveAllPublicLinksAsync consults no security context; RetrieveAllLinksAsync widens with the caller. The two return the same type and the same shape, so a wiring slip between them compiles, passes every attribute test, and leaks unapproved drafts to anonymous visitors.
        /// </summary>
        [Fact]
        public async Task ShouldReturnLinksOnGetPublicAsync()
        {
            // given
            ...
            linkProcessingServiceMock
                .Setup(service => service.RetrieveAllPublicLinksAsync(It.IsAny<CancellationToken>()))
            ...
            linkProcessingServiceMock
                .Verify(service => service.RetrieveAllPublicLinksAsync(It.IsAny<CancellationToken>()),
                    Times.Once);
