    ...
    public partial class LinksControllerTests
    {
        ...
        public async Task ShouldRemoveRecordOnDeleteByIdsAsync()
        {
            ...
            linkProcessingServiceMock
                .Setup(service => service.RemoveLinkByIdAsync(
                    It.IsAny<Guid>(),
                    inputDeletionReason,
            ...
            ActionResult<Link> actualActionResult =
                await linksController.DeleteLinkByIdAsync(randomLink.Id, inputDeletionReason, TestContext.Current.CancellationToken);

            // then
            actualActionResult.ShouldBeEquivalentTo(expectedActionResult);

            linkProcessingServiceMock
                .Verify(service => service.RemoveLinkByIdAsync(
                    It.IsAny<Guid>(),
                    inputDeletionReason,
                    It.IsAny<CancellationToken>()),
                        Times.Once);
