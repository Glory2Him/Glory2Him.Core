...
namespace Glory2Him.WebApp.Tests.Unit.Controllers.Comments
{
    public partial class CommentsControllerTests
    {
        ...
        public async Task ShouldReturnRecordsOnGetAsync()
        {
            ...
            IQueryable<Comment> storageComments = randomComments.DeepClone();
            IQueryable<Comment> expectedComment = storageComments.DeepClone();

            var expectedObjectResult =
                new OkObjectResult(expectedComment);

            var expectedActionResult =
                new ActionResult<IQueryable<Comment>>(expectedObjectResult);

            commentServiceMock
                .Setup(service => service.RetrieveAllCommentsAsync(It.IsAny<CancellationToken>()))
            ...
            commentServiceMock
               .Verify(service => service.RetrieveAllCommentsAsync(It.IsAny<CancellationToken>()),
                   Times.Once);
