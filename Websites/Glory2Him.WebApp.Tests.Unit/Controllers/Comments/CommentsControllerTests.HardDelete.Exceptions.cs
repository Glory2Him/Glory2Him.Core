    ...
    public partial class CommentsControllerTests
    {
            ...
            Xeption validationException)
        {
            ...
            this.commentServiceMock.Setup(service =>
                service.HardRemoveCommentByIdAsync(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
                    .ThrowsAsync(validationException);

            // when
            ActionResult<Comment> actualActionResult =
                await this.commentsController.HardDeleteCommentByIdAsync(someId, TestContext.Current.CancellationToken);
        ...
        [Fact]
        public async Task ShouldReturnConflictOnHardDeleteIfAlreadyExistsCommentErrorOccurredAsync()
        {
            // given
            Guid someId = Guid.NewGuid();
            var someInnerException = new Exception();
