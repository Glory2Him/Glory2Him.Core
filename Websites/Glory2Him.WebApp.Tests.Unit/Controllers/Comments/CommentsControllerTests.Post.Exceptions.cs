    ...
    public partial class CommentsControllerTests
    {
        ...
        public async Task ShouldReturnBadRequestOnPostIfValidationErrorOccurredAsync(Xeption validationException)
        {
            ...
            this.commentServiceMock.Setup(service =>
                service.AddCommentAsync(It.IsAny<Comment>(), It.IsAny<CancellationToken>()))
                    .ThrowsAsync(validationException);

            // when
            ActionResult<Comment> actualActionResult =
                await this.commentsController.PostCommentAsync(someComment, TestContext.Current.CancellationToken);
        ...
        [Fact]
        public async Task ShouldReturnConflictOnPostIfAlreadyExistsCommentErrorOccurredAsync()
        {
            // given
            Comment someComment = CreateRandomComment();
