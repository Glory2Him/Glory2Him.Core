    ...
    public partial class CommentsControllerTests
    {
            ...
            Xeption validationException)
        {
            ...
            this.commentServiceMock.Setup(service =>
                service.SubmitCommentByIdAsync(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
                    .ThrowsAsync(validationException);

            // when
            ActionResult<Comment> actualActionResult =
                await this.commentsController.SubmitCommentByIdAsync(someId, TestContext.Current.CancellationToken);
        ...
        [Fact]
        public async Task ShouldReturnConflictOnSubmitIfAlreadyExistsCommentErrorOccurredAsync()
        {
            // given
            Guid someId = Guid.NewGuid();
            var someInnerException = new Exception();
