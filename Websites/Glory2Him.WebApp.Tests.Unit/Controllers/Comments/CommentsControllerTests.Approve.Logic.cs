...
namespace Glory2Him.WebApp.Tests.Unit.Controllers.Comments
{
    public partial class CommentsControllerTests
    {
        ...
        public async Task ShouldReturnOkOnApproveAsync()
        {
            ...
            Comment storageComment = inputComment.DeepClone();
            ...
            commentServiceMock
                .Setup(service => service.TransitionCommentApprovalAsync(inputComment, It.IsAny<CancellationToken>()))
                    .ReturnsAsync(storageComment);

            // when
            ActionResult<Comment> actualActionResult = await commentsController.TransitionCommentApprovalAsync(randomComment, TestContext.Current.CancellationToken);

            // then
            actualActionResult.ShouldBeEquivalentTo(expectedActionResult);

            commentServiceMock
               .Verify(service => service.TransitionCommentApprovalAsync(inputComment, It.IsAny<CancellationToken>()),
                   Times.Once);
