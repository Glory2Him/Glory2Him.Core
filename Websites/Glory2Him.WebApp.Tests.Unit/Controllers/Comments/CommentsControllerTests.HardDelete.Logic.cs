...
namespace Glory2Him.WebApp.Tests.Unit.Controllers.Comments
{
    public partial class CommentsControllerTests
    {
        ...
        public async Task ShouldRemoveRecordOnHardDeleteByIdsAsync()
        {
            ...
            commentServiceMock
                .Setup(service => service.HardRemoveCommentByIdAsync(
                    It.IsAny<Guid>(),
            ...
            ActionResult<Comment> actualActionResult =
                await commentsController.HardDeleteCommentByIdAsync(randomComment.Id, TestContext.Current.CancellationToken);

            // then
            actualActionResult.ShouldBeEquivalentTo(expectedActionResult);

            commentServiceMock
                .Verify(service => service.HardRemoveCommentByIdAsync(
                    It.IsAny<Guid>(),
                    It.IsAny<CancellationToken>()),
                        Times.Once);
