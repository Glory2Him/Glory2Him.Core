...
namespace Glory2Him.WebApp.Tests.Unit.Controllers.Tags
{
    public partial class TagsControllerTests
    {
        ...
        public async Task ShouldReturnOkOnPutAsync()
        {
            ...
            tagServiceMock
                .Setup(service => service.ModifyTagAsync(inputTag, It.IsAny<CancellationToken>()))
                    .ReturnsAsync(storageTag);

            // when
            ActionResult<Tag> actualActionResult = await tagsController.PutTagAsync(randomTag, TestContext.Current.CancellationToken);

            // then
            actualActionResult.ShouldBeEquivalentTo(expectedActionResult);

            tagServiceMock
               .Verify(service => service.ModifyTagAsync(inputTag, It.IsAny<CancellationToken>()),
                   Times.Once);
