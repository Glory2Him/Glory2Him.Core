    ...
    public partial class TagsControllerTests
    {
        ...
        public async Task ShouldReturnBadRequestOnPutIfValidationErrorOccurredAsync(Xeption validationException)
        {
            ...
            this.tagServiceMock.Setup(service =>
                service.ModifyTagAsync(It.IsAny<Tag>(), It.IsAny<CancellationToken>()))
                    .ThrowsAsync(validationException);

            // when
            ActionResult<Tag> actualActionResult =
                await this.tagsController.PutTagAsync(someTag, TestContext.Current.CancellationToken);
        ...
        [Fact]
        public async Task ShouldReturnConflictOnPutIfAlreadyExistsTagErrorOccurredAsync()
        {
            // given
            Tag someTag = CreateRandomTag();
