    ...
    public partial class TagsControllerTests
    {
            ...
            Xeption validationException)
        {
            ...
            this.tagServiceMock.Setup(service =>
                service.SubmitTagByIdAsync(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
                    .ThrowsAsync(validationException);

            // when
            ActionResult<Tag> actualActionResult =
                await this.tagsController.SubmitTagByIdAsync(someId, TestContext.Current.CancellationToken);
        ...
        [Fact]
        public async Task ShouldReturnConflictOnSubmitIfAlreadyExistsTagErrorOccurredAsync()
        {
            // given
            Guid someId = Guid.NewGuid();
            var someInnerException = new Exception();
