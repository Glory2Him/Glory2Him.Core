    ...
    public partial class ApprovalSettingsControllerTests
    {
            ...
            Xeption validationException)
        {
            ...
            this.approvalSettingServiceMock.Setup(service =>
                service.HardRemoveApprovalSettingByIdAsync(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
                    .ThrowsAsync(validationException);

            // when
            ActionResult<ApprovalSetting> actualActionResult =
                await this.approvalSettingsController.HardDeleteApprovalSettingByIdAsync(someId, TestContext.Current.CancellationToken);
        ...
        [Fact]
        public async Task ShouldReturnConflictOnHardDeleteIfAlreadyExistsApprovalSettingErrorOccurredAsync()
        {
            // given
            Guid someId = Guid.NewGuid();
            var someInnerException = new Exception();
