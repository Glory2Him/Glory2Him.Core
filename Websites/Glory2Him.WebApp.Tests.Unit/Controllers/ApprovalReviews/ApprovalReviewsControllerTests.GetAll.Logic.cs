...
namespace Glory2Him.WebApp.Tests.Unit.Controllers.ApprovalReviews
{
    public partial class ApprovalReviewsControllerTests
    {
        ...
        public async Task ShouldReturnRecordsOnGetAsync()
        {
            ...
            IQueryable<ApprovalReview> storageApprovalReviews = randomApprovalReviews.DeepClone();
            IQueryable<ApprovalReview> expectedApprovalReviews = storageApprovalReviews.DeepClone();

            var expectedObjectResult =
                new OkObjectResult(expectedApprovalReviews);

            var expectedActionResult =
                new ActionResult<IQueryable<ApprovalReview>>(expectedObjectResult);
            ...
            ActionResult<IQueryable<ApprovalReview>> actualActionResult =
                await approvalReviewsController.Get(TestContext.Current.CancellationToken);

            // then
            actualActionResult.ShouldBeEquivalentTo(expectedActionResult);

            approvalReviewServiceMock
                .Verify(service => service.RetrieveAllApprovalReviewsAsync(
                    It.IsAny<CancellationToken>()),
                        Times.Once);
