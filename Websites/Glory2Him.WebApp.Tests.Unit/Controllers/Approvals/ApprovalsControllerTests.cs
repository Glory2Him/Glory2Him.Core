...
namespace Glory2Him.WebApp.Tests.Unit.Controllers.Approvals
{
    ...
    /// suites use — three actions is not enough to earn the split. The three security theories at
    ...
    /// was added alongside.
    ///
    /// <para>THREE, not eight: the five reviewer-coordination routes — candidates, display names
    /// and the three review-request verbs — moved to <c>ApprovalReviewersController</c> (#523,
    /// §12.5.4 business rule 6), which binds <c>IApprovalReviewerOrchestrationService</c> alone.
    /// They are asserted against their own exposer and their own service in
    /// <c>Controllers/ApprovalReviewers/ApprovalReviewersControllerTests</c>, which carries
    /// security theories of the same shape over its own inventory. Berean's three actions left the
    /// same way, earlier, into
    /// <c>Controllers/AIReviewers/AIReviewersControllerTests</c>.</para>
    /// </summary>
