    ...
    public partial class ContentItemsControllerTests
    {
        ...
        public async Task ShouldReturnCreatedOnPostAsync()
        {
            ...
            var expectedObjectResult =
                new CreatedObjectResult(expectedContentItem);

            var expectedActionResult =
                new ActionResult<ContentItem>(expectedObjectResult);

            contentItemProcessingServiceMock
                .Setup(service => service.AddContentItemAsync(inputContentItem, It.IsAny<CancellationToken>()))
                    .ReturnsAsync(addedContentItem);

            // when
            ActionResult<ContentItem> actualActionResult = await contentItemsController.PostContentItemAsync(randomContentItem, TestContext.Current.CancellationToken);

            // then
            actualActionResult.ShouldBeEquivalentTo(expectedActionResult);

            contentItemProcessingServiceMock
               .Verify(service => service.AddContentItemAsync(inputContentItem, It.IsAny<CancellationToken>()),
                   Times.Once);
