...
namespace Glory2Him.WebApp.Tests.Unit.Controllers.ContentItems
{
    public partial class ContentItemsControllerTests
    {
        ...
        public async Task ShouldReturnRecordsOnGetAsync()
        {
            ...
            IQueryable<ContentItem> storageContentItems = randomContentItems.DeepClone();
            IQueryable<ContentItem> expectedContentItem = storageContentItems.DeepClone();

            var expectedObjectResult =
                new OkObjectResult(expectedContentItem);

            var expectedActionResult =
                new ActionResult<IQueryable<ContentItem>>(expectedObjectResult);

            contentItemProcessingServiceMock
                .Setup(service => service.RetrieveAllContentItemsAsync(It.IsAny<CancellationToken>()))
            ...
            contentItemProcessingServiceMock
               .Verify(service => service.RetrieveAllContentItemsAsync(It.IsAny<CancellationToken>()),
                   Times.Once);
