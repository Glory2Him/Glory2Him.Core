    ...
    public partial class ContentItemsControllerTests
    {
        ...
        public async Task ShouldReturnOkOnPutAsync()
        {
            ...
            contentItemProcessingServiceMock
                .Setup(service => service.ModifyContentItemAsync(inputContentItem, It.IsAny<CancellationToken>()))
                    .ReturnsAsync(storageContentItem);

            // when
            ActionResult<ContentItem> actualActionResult = await contentItemsController.PutContentItemAsync(randomContentItem, TestContext.Current.CancellationToken);

            // then
            actualActionResult.ShouldBeEquivalentTo(expectedActionResult);

            contentItemProcessingServiceMock
               .Verify(service => service.ModifyContentItemAsync(inputContentItem, It.IsAny<CancellationToken>()),
                   Times.Once);
