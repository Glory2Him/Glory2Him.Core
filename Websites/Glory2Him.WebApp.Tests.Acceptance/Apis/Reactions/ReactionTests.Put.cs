...
namespace Glory2Him.WebApp.Tests.Acceptance.Apis.Reactions
{
    ...
    {
        [Fact]
        public async Task ShouldPutReactionAsync()
        {
            // given
            Reaction randomReaction =
                await PostRandomReactionAsync();

            Reaction modifiedReaction =
                UpdateReactionWithRandomValues(randomReaction);

            try
            {
                // when
                await this.apiBroker.PutReactionAsync(modifiedReaction);

                Reaction actualReaction = await this.apiBroker
                    .GetReactionByIdAsync(randomReaction.Id);

                // then
                actualReaction.Should().BeEquivalentTo(modifiedReaction, options => options
