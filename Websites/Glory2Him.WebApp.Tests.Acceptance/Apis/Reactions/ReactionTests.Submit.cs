...
namespace Glory2Him.WebApp.Tests.Acceptance.Apis.Reactions
{
    ...
    {
        [Fact]
        public async Task ShouldSubmitReactionByIdAsync()
        {
            // given
            ...
            {
                // when
                await this.apiBroker.SubmitReactionByIdAsync(randomReaction.Id);

                Reaction actualReaction = await this.apiBroker
                    .GetReactionByIdAsync(randomReaction.Id);

                // then
                actualReaction.ApprovalStatus.Should().Be(ApprovalStatus.Submitted);
            }
            finally
            {
