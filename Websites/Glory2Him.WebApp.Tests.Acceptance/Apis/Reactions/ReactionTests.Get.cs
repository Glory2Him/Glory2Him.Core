...
namespace Glory2Him.WebApp.Tests.Acceptance.Apis.Reactions
{
    public partial class ReactionApiTests
    {
        ...
        {
            // given
            List<Reaction> randomReactions = await PostRandomReactionsAsync();
            List<Reaction> expectedReactions = randomReactions;
            ...
            try
            {
                foreach (Reaction expectedReaction in expectedReactions)
                {
                    Reaction actualReaction =
                        actualReactions.Single(reaction => reaction.Id == expectedReaction.Id);
            ...
            finally
            {
                ...
                foreach (Reaction postedReaction in randomReactions)
                {
                    await this.apiBroker.RemoveCoreReactionByIdAsync(postedReaction.Id);
                }
            }
        }
    }
