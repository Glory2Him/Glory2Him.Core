...
namespace Glory2Him.WebApp.Tests.Acceptance.Apis.Tags
{
    ...
    public partial class TagApiTests
    {
        ...
        /// <summary>
        /// Modify is owner-OR-the-PUBLISHER-tier, so a publisher may correct a tag they did
        /// not create while it is in flight. Both tiers are exercised — the global
        /// <c>Publishers</c> and the entity-scoped <c>Tag-Publishers</c> — because the foundation
        /// tests for both and seeding only one would leave half the rule dead.
        /// </summary>
        [Theory]
        [InlineData(Roles.Publishers)]
        [InlineData(Roles.TagPublishers)]
        public async Task ShouldAllowPublisherToModifyAnotherUsersTagAsync(string publishRoleName)
        {
            // given
            Tag randomTag = await PostRandomTagAsync();
            Tag modifiedTag = UpdateTagWithRandomValues(randomTag);
        ...
        /// tier may correct someone else's tag in flight but may never delete it, and the
        ...
        public async Task ShouldReturnUnauthorizedOnDeleteIfCallerIsNeitherOwnerNorAdministratorAsync()
        {
            // given
            Tag randomTag = await PostRandomTagAsync();
            this.apiBroker.ActAs(Guid.NewGuid().ToString(), Roles.TagPublishers);
