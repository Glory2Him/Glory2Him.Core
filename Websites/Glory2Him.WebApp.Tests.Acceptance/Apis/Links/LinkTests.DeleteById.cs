...
namespace Glory2Him.WebApp.Tests.Acceptance.Apis.Links
{
    ...
    {
        [Fact]
        public async Task ShouldDeleteLinkByIdAsync()
        {
            // given
            ...
            try
            {
                // when
                Link deletedLink =
                    await this.apiBroker.DeleteLinkByIdAsync(inputLink.Id);

                List<Link> actualResult =
                    await this.apiBroker.GetSpecificLinkByIdAsync(inputLink.Id);
