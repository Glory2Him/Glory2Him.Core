...
namespace Glory2Him.WebApp.Tests.Acceptance.Apis.Links
{
    ...
    {
        [Fact]
        public async Task ShouldGetAllLinksAsync()
        {
            // given
            List<Link> randomLinks = await PostRandomLinksAsync();
            List<Link> expectedLinks = randomLinks;
            ...
            try
            {
                foreach (Link expectedLink in expectedLinks)
                {
                    Link actualLink =
                        actualLinks.Single(link => link.Id == expectedLink.Id);
            ...
            finally
            {
                ...
                foreach (Link postedLink in randomLinks)
                {
                    await this.apiBroker.RemoveCoreLinkByIdAsync(postedLink.Id);
                }
            }
        }
    }
