...
namespace Glory2Him.WebApp.Tests.Acceptance.Apis.Links
{
    ...
    {
        [Fact]
        public async Task ShouldPutLinkAsync()
        {
            // given
            Link randomLink =
                await PostRandomLinkAsync();

            Link modifiedLink =
                UpdateLinkWithRandomValues(randomLink);

            try
            {
                // when
                await this.apiBroker.PutLinkAsync(modifiedLink);

                Link actualLink = await this.apiBroker
                    .GetLinkByIdAsync(randomLink.Id);

                // then
                actualLink.Should().BeEquivalentTo(modifiedLink, options => options
