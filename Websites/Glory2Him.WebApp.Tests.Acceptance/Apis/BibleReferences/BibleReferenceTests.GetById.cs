...
namespace Glory2Him.WebApp.Tests.Acceptance.Apis.BibleReferences
{
    ...
    {
        [Fact]
        public async Task ShouldGetBibleReferenceByIdAsync()
        {
            // given
            BibleReference randomBibleReference = await PostRandomBibleReferenceAsync();
            BibleReference expectedBibleReference = randomBibleReference;

            try
            {
                // when
                BibleReference actualBibleReference =
                    await this.apiBroker.GetBibleReferenceByIdAsync(randomBibleReference.Id);
