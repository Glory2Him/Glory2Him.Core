...
namespace Glory2Him.WebApp.Tests.Acceptance.Apis.Comments
{
    ...
    {
        [Fact]
        public async Task ShouldSubmitCommentByIdAsync()
        {
            // given
            ...
            {
                // when
                await this.apiBroker.SubmitCommentByIdAsync(randomComment.Id);

                Comment actualComment = await this.apiBroker
                    .GetCommentByIdAsync(randomComment.Id);

                // then
                actualComment.ApprovalStatus.Should().Be(ApprovalStatus.Submitted);
            }
            finally
            {
