...
namespace Glory2Him.WebApp.Tests.Acceptance.Apis.ContentItemSettings
{
    ...
    {
        [Fact]
        public async Task ShouldGetAllContentItemSettingsAsync()
        {
            // given
            List<ContentItemSetting> randomContentItemSettings = await PostRandomContentItemSettingsAsync();
            List<ContentItemSetting> expectedContentItemSettings = randomContentItemSettings;

            // when
            List<ContentItemSetting> actualContentItemSettings = await this.apiBroker.GetAllContentItemSettingsAsync();

            // then
            try
            {
                foreach (ContentItemSetting expectedContentItemSetting in expectedContentItemSettings)
                {
                    ContentItemSetting actualContentItemSetting =
                        actualContentItemSettings.Single(contentItemSetting => contentItemSetting.Id == expectedContentItemSetting.Id);
            ...
            finally
            {
                ...
                foreach (ContentItemSetting postedContentItemSetting in randomContentItemSettings)
                {
                    await this.apiBroker.RemoveCoreContentItemSettingByIdAsync(postedContentItemSetting.Id);
                }
            }
        }
    }
