...
namespace Glory2Him.WebApp.Tests.Acceptance.Apis.ApprovalSettings
{
    ...
    {
        [Fact]
        public async Task ShouldGetApprovalSettingByIdAsync()
        {
            // given
            ApprovalSetting randomApprovalSetting = await PostRandomApprovalSettingAsync();
            ApprovalSetting expectedApprovalSetting = randomApprovalSetting;

            try
            {
                // when
                ApprovalSetting actualApprovalSetting =
                    await this.apiBroker.GetApprovalSettingByIdAsync(randomApprovalSetting.Id);
