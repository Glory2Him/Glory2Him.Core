...
namespace Glory2Him.Core.Services.Foundations.BibleReferences
{
    internal partial class BibleReferenceService
    {
        ...
        // audit (Reviewers, Publishers, Administrators — global or BibleReference-scoped, §16.6)
        ...
        // row they are reviewing; they simply may no longer edit it.
        //
        // The publisher tier's amendment window is Draft and Submitted, and it is bounded not
        // here but by ValidateStorageBibleReferenceIsNotTerminal on the same path — which reads the
        ...
        private async ValueTask<bool> ValidateUserCanModifyStorageBibleReferenceAsync(
        ...
        //
        // A bibleReference never forks, so refusing the write IS the enforcement — there is no version to
        ...
        private static void ValidateStorageBibleReferenceIsNotTerminal(BibleReference storageBibleReference)
        {
            bool isTerminal =
                storageBibleReference.ApprovalStatus == ApprovalStatus.Approved
                    || storageBibleReference.ApprovalStatus == ApprovalStatus.Rejected;
            ...
            SecurityContext securityContext)
        {
            ...
            Validate(
                ...
                (Rule: IsSetOnAdd(bibleReference.IsPublished),
                    Parameter: nameof(BibleReference.IsPublished)),

                (Rule: IsSetOnAdd(bibleReference.PublishDate),
                    Parameter: nameof(BibleReference.PublishDate)),

                (Rule: IsNotContributableStatus(bibleReference.ApprovalStatus),
        ...
        private async ValueTask ValidateBibleReferenceEventEnvelopeAsync(
            ...
            BibleReferenceEventOperation operation)
        {
            ...
            string eventName = $"{nameof(BibleReference)}{operation}";
            ...
            {
                throw new InvalidBibleReferenceEventException(
                    message: "Invalid bible reference event. Integrity verification failed.");
            }
        }
            ...
            bool mayTransitionApprovalStatus)
        {
            Validate(
                ...
                (Rule: IsNotAPermittedStatusChangeOnModify(
                        inputStatus: inputBibleReference.ApprovalStatus,
                        storageStatus: storageBibleReference.ApprovalStatus,
                        mayTransition: mayTransitionApprovalStatus),
                    Parameter: nameof(BibleReference.ApprovalStatus)),

                (Rule: IsNotSame(
                        first: inputBibleReference.IsPublished,
                        second: storageBibleReference.IsPublished,
                        secondName: nameof(BibleReference.IsPublished)),
                    Parameter: nameof(BibleReference.IsPublished)),
