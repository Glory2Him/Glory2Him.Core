...
namespace Glory2Him.Core.Services.Foundations.BibleReferences
{
    ...
    {
        ValueTask<EventEnvelope<BibleReference>?> OnAddingBibleReferenceAsync(
            EventEnvelope<BibleReference> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<BibleReference>?> OnModifyingBibleReferenceAsync(
            EventEnvelope<BibleReference> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<BibleReference>?> OnRemovingBibleReferenceByIdAsync(
            EventEnvelope<BibleReference> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<BibleReference>?> OnHardRemovingBibleReferenceByIdAsync(
            EventEnvelope<BibleReference> envelope,
            CancellationToken cancellationToken = default);
