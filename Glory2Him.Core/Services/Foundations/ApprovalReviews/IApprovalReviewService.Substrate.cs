...
namespace Glory2Him.Core.Services.Foundations.ApprovalReviews
{
    ...
    {
        ValueTask<EventEnvelope<ApprovalReview>?> OnAddingApprovalReviewAsync(
            EventEnvelope<ApprovalReview> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<ApprovalReview>?> OnModifyingApprovalReviewAsync(
            EventEnvelope<ApprovalReview> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<ApprovalReview>?> OnRemovingApprovalReviewByIdAsync(
            EventEnvelope<ApprovalReview> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<ApprovalReview>?> OnHardRemovingApprovalReviewByIdAsync(
            EventEnvelope<ApprovalReview> envelope,
            CancellationToken cancellationToken = default);
