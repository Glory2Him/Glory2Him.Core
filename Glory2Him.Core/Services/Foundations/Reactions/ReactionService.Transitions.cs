...
namespace Glory2Him.Core.Services.Foundations.Reactions
{
    ...
    /// <c>DoModifyReactionAsync</c> in one important way: the row is loaded FIRST and the caller's
    ...
    {
        public ValueTask<Reaction> SubmitReactionByIdAsync(
            Guid reactionId,
        ...
        public ValueTask<Reaction> TransitionReactionApprovalAsync(
            ...
            TryCatch(async () =>
            {
                ...
                return await DoTransitionReactionApprovalAsync(
                    ...
                    // The event path admits the claim too, but only because OnApprovingReactionAsync
            ...
            CancellationToken cancellationToken)
        {
            ...
            // the caller supplied
            await ValidateUserCanSubmitStorageReactionAsync(
                storageReaction: storageReaction,
                securityContext: inboundEnvelope.SecurityContext);

            ValidateStorageReactionIsSubmittable(storageReaction);
            ...
            storageReaction.ApprovalStatus = ApprovalStatus.Submitted;
        ...
        private async ValueTask<Reaction> LoadTransitionTargetAsync(
            Guid reactionId,
            CancellationToken cancellationToken)
        {
            Reaction maybeReaction =
                await this.storageBroker.SelectReactionByIdAsync(
                    reactionId: reactionId,
                    cancellationToken: cancellationToken);
            ...
            ValidateStorageReactionIsNotDeleted(maybeReaction, reactionId);
        ...
        private async ValueTask<Reaction> SaveTransitionAsync(
            Reaction reaction,
            EventEnvelope<Reaction> inboundEnvelope,
            ...
            CancellationToken cancellationToken)
        {
            ...
            Reaction updatedReaction =
                await this.storageBroker.UpdateReactionAsync(
                    reaction,
                    cancellationToken);
