...
namespace Glory2Him.Core.Services.Foundations.Reactions
{
    ...
    public partial interface IReactionService
    {
        ...
        ValueTask<EventEnvelope<Reaction>?> OnModifyingReactionAsync(
            EventEnvelope<Reaction> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<Reaction>?> OnRemovingReactionByIdAsync(
            EventEnvelope<Reaction> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<Reaction>?> OnHardRemovingReactionByIdAsync(
            EventEnvelope<Reaction> envelope,
            CancellationToken cancellationToken = default);
