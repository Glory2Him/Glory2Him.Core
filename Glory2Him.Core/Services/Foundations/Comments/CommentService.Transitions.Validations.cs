...
namespace Glory2Him.Core.Services.Foundations.Comments
{
    ...
    {
        private static void ValidateOnSubmitComment(Guid commentId) =>
            Validate(
                message: "Comment is invalid, fix the errors and try again.",
                (Rule: IsInvalid(commentId), Parameter: nameof(Comment.Id)));

        private static void ValidateOnTransitionCommentApproval(Comment comment) =>
            Validate(
                message: "Comment is invalid, fix the errors and try again.",
                ...
                (Rule: IsNotAnApprovalTransitionTarget(comment.ApprovalStatus),
                ...
                (Rule: IsPublishedWithoutApproval(
                        comment.ApprovalStatus, comment.IsPublished),
                    Parameter: nameof(Comment.IsPublished)),

                (Rule: IsPublishDateWithoutPublication(
                        comment.IsPublished, comment.PublishDate),
                ...
                (Rule: IsBypassWithoutApproval(
                        comment.IsApprovedByBypass, comment.ApprovalStatus),
                ...
                (Rule: IsMissingBypassReason(
                        comment.IsApprovedByBypass, comment.ApprovedByBypassReason),
                ...
                // field at all.
                (Rule: IsGreaterThan(comment.ApprovedByBypassReason, 500),
                    Parameter: nameof(Comment.ApprovedByBypassReason)));
        ...
        // because this is the only path by which a comment becomes publicly visible.
        //
        ...
        private async ValueTask<bool> ValidateUserCanTransitionStorageCommentApprovalAsync(
            Comment storageComment,
            Comment comment,
            SecurityContext securityContext,
            ...
            CancellationToken cancellationToken)
        {
            ...
            bool isOverride =
                storageComment.ApprovalStatus == ApprovalStatus.Approved
                    || storageComment.ApprovalStatus == ApprovalStatus.Rejected;
            ...
            if (comment.ApprovalStatus == ApprovalStatus.Submitted)
            {
                return false;
            }
            ...
            AccessVerdict verdict = await this.accessBroker.MayDecideApprovalAsync(
                new ApprovalDecisionQuery
                {
                    ...
                    IsSubjectDeleted = storageComment.IsDeleted,

                    // A comment carries no content type, so its policy tier is (Comment, null) — the
                    // same shape an association uses. There is exactly one tier to resolve
                    // against.
                    ContentType = null,

                    // One subject: the comment authorises from itself, keyed by its own type with
                    // no content type.
                    RoleSubjects = new List<RoleSubject>
                    {
                        new RoleSubject
                        {
                            EntityType = EntityType.Comment.ToString(),
                            ContentType = null,
                        },
                    },
                    ...
                    EntityCreatedBy = storageComment.CreatedBy,
                    ...
                    IsBypassRequested = comment.IsApprovedByBypass,
                    BypassReason = comment.ApprovedByBypassReason,
            ...
            if (verdict.IsPermitted is false)
            {
                ...
                await this.loggingBroker.LogWarningAsync(
                    $"Comment approval denied for {storageComment.Id}. "
        ...
        private static void ValidateStorageCommentIsTransitionable(
            Comment storageComment)
        {
        ...
        private static void ValidateStorageCommentIsSubmittable(
            Comment storageComment)
        {
            if (storageComment.ApprovalStatus != ApprovalStatus.Draft)
            {
                throw new InvalidCommentException(
        ...
        // transitions become a probe for which comments used to exist.
        private static void ValidateStorageCommentIsNotDeleted(
            Comment storageComment,
            Guid commentId)
        {
            if (storageComment.IsDeleted)
            {
        ...
        private static dynamic IsBypassWithoutApproval(
            ...
            ApprovalStatus approvalStatus) => new
            {
                ...
                Message = "Bypass requires an approved comment."
            };
        ...
        private static dynamic IsPublishedWithoutApproval(
            ...
            bool isPublished) => new
            {
                ...
                Message = "Is published requires an approved comment."
            };
        ...
        private static dynamic IsPublishDateWithoutPublication(
            ...
            DateTimeOffset? publishDate) => new
            {
                ...
                Message = "Publish date requires a published comment."
            };
    }
}
