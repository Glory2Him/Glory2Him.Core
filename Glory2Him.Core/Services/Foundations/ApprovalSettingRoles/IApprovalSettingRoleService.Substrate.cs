...
namespace Glory2Him.Core.Services.Foundations.ApprovalSettingRoles
{
    ...
    {
        ValueTask<EventEnvelope<ApprovalSettingRole>?> OnAddingApprovalSettingRoleAsync(
            EventEnvelope<ApprovalSettingRole> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<ApprovalSettingRole>?> OnModifyingApprovalSettingRoleAsync(
            EventEnvelope<ApprovalSettingRole> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<ApprovalSettingRole>?> OnRemovingApprovalSettingRoleByIdAsync(
            EventEnvelope<ApprovalSettingRole> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<ApprovalSettingRole>?> OnHardRemovingApprovalSettingRoleByIdAsync(
            EventEnvelope<ApprovalSettingRole> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<ApprovalSettingRole>?> OnRetrievingApprovalSettingRoleByIdAsync(
            EventEnvelope<ApprovalSettingRole> envelope,
            CancellationToken cancellationToken = default);
    }
}
