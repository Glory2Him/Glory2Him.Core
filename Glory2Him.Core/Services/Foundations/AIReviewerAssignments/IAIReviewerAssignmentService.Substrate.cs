...
namespace Glory2Him.Core.Services.Foundations.AIReviewerAssignments
{
    ...
    {
        ValueTask<EventEnvelope<AIReviewerAssignment>?> OnAddingAIReviewerAssignmentAsync(
            EventEnvelope<AIReviewerAssignment> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<AIReviewerAssignment>?> OnModifyingAIReviewerAssignmentAsync(
            EventEnvelope<AIReviewerAssignment> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<AIReviewerAssignment>?> OnRemovingAIReviewerAssignmentByIdAsync(
            EventEnvelope<AIReviewerAssignment> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<AIReviewerAssignment>?> OnRetrievingAIReviewerAssignmentByIdAsync(
            EventEnvelope<AIReviewerAssignment> envelope,
            CancellationToken cancellationToken = default);
    }
}
