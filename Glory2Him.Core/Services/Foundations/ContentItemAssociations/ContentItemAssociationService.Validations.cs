...
namespace Glory2Him.Core.Services.Foundations.ContentItemAssociations
{
    internal partial class ContentItemAssociationService
    {
        ...
        private static void ValidateUserIsAllowedToContribute(SecurityContext securityContext)
        {
            ...
            {
                throw new UnauthorizedContentItemAssociationException(
                    message: "The current user is not authenticated.");
            }

            bool isBlocked =
                securityContext.Roles.Contains(Roles.ReadOnly)
                    || securityContext.Roles.Contains(Roles.ContentItemAssociationReadOnly);
        ...
        // audit (Reviewer, Publisher, Admin — global or ContentItemAssociation-scoped, §16.6)
        private static bool HasReviewRole(SecurityContext securityContext) =>
            securityContext.Roles.Contains(Roles.Reviewer)
                || securityContext.Roles.Contains(Roles.ContentItemAssociationReviewer)
                || securityContext.Roles.Contains(Roles.Publisher)
                || securityContext.Roles.Contains(Roles.ContentItemAssociationPublisher)
        ...
        private async ValueTask ValidateUserCanModifyStorageContentItemAssociationAsync(
            ...
            SecurityContext securityContext)
        {
            ...
            bool isOwner =
                string.IsNullOrWhiteSpace(actorUserId) is false
                    && storageContentItemAssociation.CreatedBy == actorUserId;
        ...
        private async ValueTask ValidateUserCanRemoveStorageContentItemAssociationAsync(
            ContentItemAssociation storageContentItemAssociation,
            SecurityContext securityContext)
        {
            string actorUserId = await this.securityAuditBroker.GetUserIdAsync(securityContext);
        ...
        private static void ValidateUserCanHardRemoveContentItemAssociation(SecurityContext securityContext)
        {
            if (securityContext is null || securityContext.IsAuthenticated is false)
            {
                throw new UnauthorizedContentItemAssociationException(
