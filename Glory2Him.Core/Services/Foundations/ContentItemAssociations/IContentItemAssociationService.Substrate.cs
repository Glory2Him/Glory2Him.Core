...
namespace Glory2Him.Core.Services.Foundations.ContentItemAssociations
{
    ...
    public partial interface IContentItemAssociationService
    {
        ...
        ValueTask<EventEnvelope<ContentItemAssociation>?> OnModifyingContentItemAssociationAsync(
            EventEnvelope<ContentItemAssociation> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<ContentItemAssociation>?> OnRemovingContentItemAssociationByIdAsync(
            EventEnvelope<ContentItemAssociation> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<ContentItemAssociation>?> OnHardRemovingContentItemAssociationByIdAsync(
            EventEnvelope<ContentItemAssociation> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<ContentItemAssociation>?> OnRetrievingContentItemAssociationByIdAsync(
            EventEnvelope<ContentItemAssociation> envelope,
            CancellationToken cancellationToken = default);
    }
}
