...
namespace Glory2Him.Core.Services.Foundations.Approvals
{
    ...
    {
        ValueTask<EventEnvelope<Approval>?> OnAddingApprovalAsync(
            EventEnvelope<Approval> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<Approval>?> OnModifyingApprovalAsync(
            EventEnvelope<Approval> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<Approval>?> OnRemovingApprovalByIdAsync(
            EventEnvelope<Approval> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<Approval>?> OnHardRemovingApprovalByIdAsync(
            EventEnvelope<Approval> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<Approval>?> OnRetrievingApprovalByIdAsync(
            EventEnvelope<Approval> envelope,
            CancellationToken cancellationToken = default);
    }
}
