...
namespace Glory2Him.Core.Services.Foundations.ApprovalComments
{
    ...
    public partial interface IApprovalCommentService
    {
        ...
        ValueTask<EventEnvelope<ApprovalComment>?> OnModifyingApprovalCommentAsync(
            EventEnvelope<ApprovalComment> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<ApprovalComment>?> OnRemovingApprovalCommentByIdAsync(
            EventEnvelope<ApprovalComment> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<ApprovalComment>?> OnHardRemovingApprovalCommentByIdAsync(
            EventEnvelope<ApprovalComment> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<ApprovalComment>?> OnRetrievingApprovalCommentByIdAsync(
            EventEnvelope<ApprovalComment> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<ApprovalComment>?> OnResolvingApprovalCommentAsync(
            EventEnvelope<ApprovalComment> envelope,
            CancellationToken cancellationToken = default);
    }
}
