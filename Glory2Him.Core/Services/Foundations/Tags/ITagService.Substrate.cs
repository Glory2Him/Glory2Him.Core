...
namespace Glory2Him.Core.Services.Foundations.Tags
{
    ...
    public partial interface ITagService
    {
        ...
        ValueTask<EventEnvelope<Tag>?> OnModifyingTagAsync(
            EventEnvelope<Tag> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<Tag>?> OnRemovingTagByIdAsync(
            EventEnvelope<Tag> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<Tag>?> OnHardRemovingTagByIdAsync(
            EventEnvelope<Tag> envelope,
            CancellationToken cancellationToken = default);
