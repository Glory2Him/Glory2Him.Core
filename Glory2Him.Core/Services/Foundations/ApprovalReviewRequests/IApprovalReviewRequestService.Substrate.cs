...
namespace Glory2Him.Core.Services.Foundations.ApprovalReviewRequests
{
    ...
    {
        ValueTask<EventEnvelope<ApprovalReviewRequest>?> OnAddingApprovalReviewRequestAsync(
            EventEnvelope<ApprovalReviewRequest> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<ApprovalReviewRequest>?> OnRemovingApprovalReviewRequestByIdAsync(
            EventEnvelope<ApprovalReviewRequest> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<ApprovalReviewRequest>?> OnHardRemovingApprovalReviewRequestByIdAsync(
            EventEnvelope<ApprovalReviewRequest> envelope,
            CancellationToken cancellationToken = default);

        ValueTask<EventEnvelope<ApprovalReviewRequest>?> OnRetrievingApprovalReviewRequestByIdAsync(
            EventEnvelope<ApprovalReviewRequest> envelope,
            CancellationToken cancellationToken = default);
    }
}
