...
{
    /// <summary>
    /// The operations a <c>Tag</c> event can represent — requests (present tense:
    ...
    /// <c>Tag-Adding</c>) and composes the stored event name (for example
    /// <c>"TagAdding"</c>). <see cref="HardRemoved"/> shares the
    /// <see cref="Removed"/> event address and is distinguished purely by its event name
    /// (<c>"TagHardRemoved"</c>). Entity-specific operations may be appended here
    /// (with a matching event address in <c>EventBrokerIdentifiers</c>) without affecting
    /// any other entity.
    /// </summary>
    public enum TagEventOperation
    {
        Adding,
        Modifying,
        RemovingById,
