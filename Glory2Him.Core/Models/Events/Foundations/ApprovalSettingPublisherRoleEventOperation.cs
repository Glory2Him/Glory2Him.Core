...
{
    /// <summary>
    /// The operations a <c>ApprovalSettingPublisherRole</c> event can represent — requests (present tense:
    ...
    /// <c>ApprovalSettingPublisherRole-Adding</c>) and composes the stored event name (for example
    /// <c>"ApprovalSettingPublisherRoleAdding"</c>). <see cref="HardRemoved"/> shares the
    /// <see cref="Removed"/> event address and is distinguished purely by its event name
    /// (<c>"ApprovalSettingPublisherRoleHardRemoved"</c>). Entity-specific operations may be appended
    /// here (with a matching event address in <c>EventBrokerIdentifiers</c>) without affecting
    /// any other entity.
    /// </summary>
    public enum ApprovalSettingPublisherRoleEventOperation
    {
        Adding,
        Modifying,
        RemovingById,
