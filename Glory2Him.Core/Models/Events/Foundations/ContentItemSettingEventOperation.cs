...
namespace Glory2Him.Core.Models.Events.Foundations
{
    ...
    /// <c>ContentItemSetting-Adding</c>) and composes the stored event name (for example
    /// <c>"ContentItemSettingAdding"</c>). <see cref="HardRemoved"/> shares the
    /// <see cref="Removed"/> event address and is distinguished purely by its event name
    /// (<c>"ContentItemSettingHardRemoved"</c>). Entity-specific operations may be appended here
