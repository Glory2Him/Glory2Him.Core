...
{
    /// <summary>
    /// The operations a <c>ApprovalSetting</c> event can represent — requests (present tense:
    ...
    /// <c>ApprovalSetting-Adding</c>) and composes the stored event name (for example
    /// <c>"ApprovalSettingAdding"</c>). <see cref="HardRemoved"/> shares the
    /// <see cref="Removed"/> event address and is distinguished purely by its event name
    /// (<c>"ApprovalSettingHardRemoved"</c>). Entity-specific operations may be appended here
    /// (with a matching event address in <c>EventBrokerIdentifiers</c>) without affecting
    /// any other entity.
    /// </summary>
    public enum ApprovalSettingEventOperation
    {
        Adding,
        Modifying,
        RemovingById,
