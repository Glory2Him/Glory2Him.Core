...
{
    /// <summary>
    /// The operations a <c>ContentItemAssociation</c> event can represent — requests (present
    ...
    /// <c>ContentItemAssociation-Adding</c>) and composes the stored event name (for example
    /// <c>"ContentItemAssociationAdding"</c>). <see cref="HardRemoved"/> shares the
    /// <see cref="Removed"/> event address and is distinguished purely by its event name
    /// (<c>"ContentItemAssociationHardRemoved"</c>). Entity-specific operations may be appended
    /// here (with a matching event address in <c>EventBrokerIdentifiers</c>) without affecting
    /// any other entity.
    /// </summary>
    public enum ContentItemAssociationEventOperation
    {
        Adding,
        Modifying,
        RemovingById,
