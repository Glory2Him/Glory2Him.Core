...
{
    /// <summary>
    /// The operations an <c>Approval</c> event can represent — requests (present tense:
    ...
    /// <c>Approval-Adding</c>) and composes the stored event name (for example
    /// <c>"ApprovalAdding"</c>). <see cref="HardRemoved"/> shares the
    /// <see cref="Removed"/> event address and is distinguished purely by its event name
    /// (<c>"ApprovalHardRemoved"</c>). Entity-specific operations may be appended here
