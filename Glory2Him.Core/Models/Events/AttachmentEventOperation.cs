...
{
    /// <summary>
    /// The operations a <c>Attachment</c> event can represent — requests (present tense:
    ...
    /// example <c>Attachment-Adding</c>) and composes the stored event name (for example
    /// <c>"AttachmentAdding"</c>). Entity-specific operations may be appended here (with a
    /// matching event address in <c>EventBrokerIdentifiers</c>) without affecting any other
    /// entity.
    /// </summary>
    public enum AttachmentEventOperation
    {
        Adding,
        Modifying,
        RemovingById,
