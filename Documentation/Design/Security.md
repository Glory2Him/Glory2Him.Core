...
2. **Review roles.** A global `Reviewers` / `Publishers` / `Administrators`, **or** a scoped role matching *at least one* endpoint. Each endpoint is checked at both tiers: the coarse `%EntityType%-Reviewers` / `-Publishers`, and the narrow `%EntityType%-%ContentType%-Reviewers` / `-Publishers` from the denormalised endpoint content type. One endpoint is enough because the pairing is the thing under review and the reviewer can see both ends of it; requiring both would leave every cross-type association unreviewable by anyone short of a global role.
   ...
   **What makes the carve safe is stated rather than waived.** The bar exists to stop an in-place amendment of **approved content** going public without re-review, and a repoint does change what the row denotes, so the bar genuinely reaches it. What narrows it is that the row is one reader's own statement about their own reaction, that only its owner may move it, and that the change goes back through the approval process in a round that starts with no reviews (§DOM4.5 rule 4, #685, not yet built): the reviews the old pair earned are dismissed whatever `RequireReapprovalOnChange` says, so the new pair **is** re-reviewed wherever its tier requires review. **That restores the safety being narrowed here** — the bar's purpose is preserved and only its blanket form is given up.
