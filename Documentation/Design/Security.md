...
2. **Review roles.** A global `Reviewers` / `Publishers` / `Administrators`, **or** a scoped role matching *at least one* endpoint. Each endpoint is checked at both tiers: the coarse `%EntityType%-Reviewers` / `-Publishers`, and the narrow `%EntityType%-%ContentType%-Reviewers` / `-Publishers` from the denormalised endpoint content type. One endpoint is enough because the pairing is the thing under review and the reviewer can see both ends of it; requiring both would leave every cross-type association unreviewable by anyone short of a global role.
   ...
   **The terminal bar admits ONE exception, and it is structurally constrained in §DOM4.1's form rather than waived.** It is **`UpsertPersonalAssociationAsync` (§ARC16.2.2), on a row whose `UserId` is set, acting for that same `UserId`** — the reader repointing their own reaction from Love to Moved (§DOM4.10, §DOM4.5 rule 4). Nothing else is admitted: an editorial row is out of reach because the operation refuses a null `UserId`, and one reader can never reach another's row because the `UserId` is derived from the inbound envelope and never read from the request. **`ModifyAssociationAsync` keeps the bar UNCONDITIONALLY** — `ValidateStorageAssociationIsNotTerminal` (`AssociationService.Validations.cs:289-302`) is unchanged, and the new member does not inherit it by accident, which is exactly why the exception has to be written down rather than assumed.

   **What makes the carve safe is stated rather than waived.** The bar exists to stop an in-place amendment of **approved content** going public without re-review, and a repoint does change what the row denotes, so the bar genuinely reaches it. What narrows it is that the row is one reader's own statement about their own reaction, that only its owner may move it, and that whether moving it needs re-review is **`RequireReapprovalOnChange`**'s answer (§APR8.4) rather than this exception's: whichever it is, the change goes back through the approval process (§DOM4.5 rule 4, #685): `true` dismisses the reviews the old pair earned, so the new pair **is** re-reviewed, and `false` keeps them, so they count for the new pair — the configured meaning §APR8.6's first residual states for any edit; under the seeded tier there were none. **Under `true`, that setting restores the safety being narrowed here** — the bar's purpose is preserved and only its blanket form is given up.
