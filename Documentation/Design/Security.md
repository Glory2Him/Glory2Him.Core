...
7. **A personal association is visible only to its owner and the review tier, on both read paths — reaction authorship is not published.** A row with `UserId` set (§DOM4.2) is admitted to its owner — **the account its `UserId` names, and not its `CreatedBy`** — and to the review tier of rule 2, and to nobody else: not to an anonymous caller, and not to an authenticated caller who is neither, **whatever its approval and publication state**. To everyone else a reaction exists only as §ARC16.8's aggregate. This narrows **posture A** rule 4's *public* for one kind of row and changes nothing for an editorial row, whose `CreatedBy` this rule does not decide. It decides **visibility** only: the owner test posture A rule 3 applies to removal is unchanged by it.

   **Why `UserId` and not `CreatedBy`.** Everywhere else in this design a personal row belongs to the account its `UserId` names. §ARC16.8 *"makes the reader's one personal row for a host unique on its endpoints together with `UserId`"*, and §ARC16.8.1 resolves *"the caller's one row for that (host, far-end type)"* through that same personal index to withdraw it. A visibility rule keyed on a different column would let a reader withdraw a row they could not read, or read one they could not withdraw. The two columns also **fail in opposite directions** if they ever differ: keyed on `UserId`, the row stays visible only to the account it names; keyed on `CreatedBy`, it would be shown to whoever wrote it and hidden from the person whose reaction it is.

   **Whether they can differ, measured.** No caller-facing path writes a personal row today: `AssociationOrchestrationService` sets `UserId` to null on every add, until #188 derives it from the envelope. The foundation's own add is weaker. It pins `CreatedBy` to the envelope's caller but checks `UserId` for length only, and `Association-Adding` still binds that foundation, so on that address a row could carry a `UserId` naming someone other than its writer. Nothing in the host publishes that address, so it does not happen today. That is an **absent guard, not a present one**, and #631 closes it by moving the address onto the orchestration, where `UserId` is set rather than accepted. The choice of column therefore matters in principle now and will matter in practice once personal rows are written.
