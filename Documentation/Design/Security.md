...
**The composite carries no caller term, and the endpoint half of rule 4 is answered by the endpoint's own read.** This rule still states no caller clause and none is added here. What is ruled, for #310, is *which read answers* "both endpoints are visible": the composite resolves each endpoint through that endpoint entity's own foundation read, whose posture already carries the caller term. §SEC14.7 posture A rule 4 admits the owner and the review roles to a non-public row, so a moderator still sees the pairing hanging off the `Submitted` item they moderate — not because the composite makes an exception for them, but because that item's own read admits them to it. Rule 3 needs no term of its own for the same reason: §SEC14.5 rule 3 refuses a soft-deleted row to **every** caller, `Administrators` included, so a deleted endpoint is unresolvable to everyone through that same read. The composite therefore applies to every caller unconditionally, and the orchestration re-derives none of §SEC14.7 posture A′ rule 5's role resolution — that resolution keeps its one home in the foundation's filter.

**The symmetry with the write is the argument.** §ARC12.3.1 rule 5a already binds the *write* to an endpoint "visible to the caller — public, or the caller's own, or the caller holds a review role", which is why an owner or reviewer may pair their own `Draft` endpoint. Answering rule 4 against the bare §SEC14.1 predicate instead would let a caller create a pairing they could not then read back, which is the one outcome neither rule intends. Applying it against the bare predicate *on top of* a widened collection read would do the same to a moderator, and that reading is refused here rather than left open.

**§ARC16.8's reaction summary read answers the same question the other way, and its own reason is why.** That read resolves its hosts through a caller-**independent** canonically-visible read, because it returns an aggregate rather than entity rows and its counts must not move when a reader signs in. One rule, two resolvers: the shared evaluator takes the resolved endpoint visibility as an **input** and does not decide how it was obtained.

**Two resolvers, but ONE predicate**, and the distinction is load-bearing. What may differ between two reads is the read that answers *is this endpoint visible to whoever is asking*; what may never differ is *which row is asked about*. An endpoint written `AllVersions` is answered at its **group** — §DOM4.6 rule 1 makes the effective id the read predicate, and §ARC16.8 says group level and not row level, because such an association belongs to the group rather than to the version that happened to be current when it was written — and an endpoint written `ThisVersionOnly` is answered at its row. A collection read matching the group while the single read of the same row matches the key id is two predicates wearing one rule's name, and it shows as a row that is in the list and not found when it is opened. Both paths reach the group through the group-keyed read the endpoint's own foundation already exposes, which carries that foundation's read posture unchanged.
...
5. Privilege denials (an anonymous caller, or an authenticated caller who is neither the owner nor in a review role, requesting a non-public entity) are logged as **warnings**, including the entity id and — when resolved — the denied user's id. These are the security-relevant events: repeated warnings for one caller indicate probing.
6. State-based misses (soft-deleted entity requested; a group with no non-deleted latest or published version) are logged as **information**, including the entity or group id.
7. The log message states the real reason and notes that the caller was answered with not-found, e.g. `Content item read denied. Content item {id} is not publicly visible and user "{userId}" is neither the owner nor in a review role; reported to the caller as not found.`

**Whether naming the reason is a disclosure turns on WHO SUPPLIED THE VALUE, and the same condition can be both.** Rule 2 bars the denial reason, the entity's state and the caller's identity from anything that surfaces outward. A value the **caller themselves supplied** is none of the three: telling them their own input was refused discloses only what they already sent, which is why §ARC16.2.1 may name the switch that refused a write on a row the caller can already see. A value read off a **stored** row is the opposite, however the message is worded. An unsupported association endpoint type is the case that makes the distinction concrete: on the add the caller supplied the endpoint, so refusing it by name is a plain validation failure; on a read keyed by association id the caller supplied only that id, so the same sentence confirms that the row exists and reports one of its columns. On a read it is therefore **not found**, on the same terms as any other endpoint that cannot be resolved — the collection read drops the row and the single read answers not-found, and the two paths agree.

**Indistinguishability is a property of the OUTWARD ANSWER, not of the exception family.** Rule 1's test is what an unprivileged probe can tell apart, and a probe sees a status code and a body. Where a read's misses arise at two layers — the entity foundation's own not-found, and a composite refusal above it — the two leave as two families, because the exception contract classifies a downstream refusal as a dependency validation failure and a locally raised one as a validation failure, and collapsing them would misreport which layer refused. What rule 1 requires is that every miss carries the **same outward message**, with no reason, no state and no identity in it, and that the exposer maps every one of them to the **same status code and the same body**. The obligation sits on the messages and on the exposer; it does not oblige a service to forge a family it did not raise.
...
2. **Review roles.** A global `Reviewers` / `Publishers` / `Administrators`, **or** a scoped role matching *at least one* endpoint. Each endpoint is checked at both tiers: the coarse `%EntityType%-Reviewers` / `-Publishers`, and the narrow `%EntityType%-%ContentType%-Reviewers` / `-Publishers` from the denormalised endpoint content type. One endpoint is enough because the pairing is the thing under review and the reviewer can see both ends of it; requiring both would leave every cross-type association unreviewable by anyone short of a global role.
   ...
   **The terminal bar admits ONE exception, and it is structurally constrained in §DOM4.1's form rather than waived.** It is **`UpsertPersonalAssociationAsync` (§ARC16.2.2), on a row whose `UserId` is set, acting for that same `UserId`** — the reader repointing their own reaction from Love to Moved (§DOM4.10, §DOM4.5 rule 4). Nothing else is admitted: an editorial row is out of reach because the operation refuses a null `UserId`, and one reader can never reach another's row because the `UserId` is derived from the inbound envelope and never read from the request. **`ModifyAssociationAsync` keeps the bar UNCONDITIONALLY** — `ValidateStorageAssociationIsNotTerminal` (`AssociationService.Validations.cs:287-300`) is unchanged, and the new member does not inherit it by accident, which is exactly why the exception has to be written down rather than assumed.

   **What makes the carve safe is stated rather than waived.** The bar exists to stop an in-place amendment of **approved content** going public without re-review, and a repoint does change what the row denotes, so the bar genuinely reaches it. What narrows it is that the row is one reader's own statement about their own reaction, that only its owner may move it, and that whether moving it needs re-review is **`RequireReapprovalOnChange`**'s answer (§APR8.4) rather than this exception's: `true` re-opens the round and the new pair **is** re-reviewed, and `false` means there was no review to bypass in the first place. **That setting is what restores the safety being narrowed here** — the bar's purpose is preserved and only its blanket form is given up.
...
4. **The gate splits on the remove and hard-remove paths.** Removal is handed an id, not an association, so the endpoint half of the veto cannot run until the row is loaded. Authentication and the global block still run first, so an anonymous or globally blocked caller never reaches the `Associations` table and cannot use these surfaces to probe which association ids exist. (On the event path a deduplication lookup against `ProcessedEvents` precedes the gate; it is keyed on the event id, not the association id, so it reveals nothing about which rows exist.) Hard removal is `Administrators` only **and** subject to the same endpoint veto — a block that stopped the reversible takedown but not the irreversible one would be the wrong way round.
   ...
   **The split is also the LAYER split, and modify is in it.** Where an orchestration composes this foundation for a write, the half that needs no row is the orchestration's and the half that needs the stored row is the foundation's, and the orchestration issues **no second read** to duplicate the latter. Modify belongs with the two removals rather than with the add: authorization here is decided against the **stored** endpoints and never the caller's copy, so a caller-supplied `Association` puts no trustworthy endpoint in the orchestration's hand either. The add is the exception that proves the rule — it resolves both endpoints from storage as its own first act, so the endpoint half is decidable there and is decided there.

   So the orchestration runs authentication, the global `ReadOnly` block, and — on hard remove — the `Administrators` check, before any read of the `Associations` table, which is what keeps this rule's probe guarantee at the layer a caller actually reaches. Everything composed from the stored endpoints runs in the foundation beneath it: each end's `%EntityType%-ReadOnly`, each `ContentItem` end's narrow `ContentItem-%ContentType%-ReadOnly`, the owner test on remove, and the endpoint-derived `Publishers` tier on modify.

   **That is the rule, and on `ModifyAssociationAsync` the foundation does not yet meet it — recorded as a gap rather than left as a claim.** The rule is not new here: posture A rule 1 already says that on every write path but the add the content type "comes off the **stored** row". Remove and hard remove compose the veto that way. Modify composes all four `ReadOnly` names from the **caller's copy**, before the storage read, and nothing after that read re-runs them; only the `Publishers` tier is stored-row composed on that path today.

   **What the gap costs, stated precisely, because the obvious reading overstates it.** No write is admitted that a stored-row veto would have refused. The general modify pins all eight endpoint fields against storage — both types, both key ids, both group ids and both content types — so any payload surviving the pin carries endpoints identical to the stored row's, and a veto composed from it is the same veto. What the order actually costs is the **refusal's identity**: a blocked caller who names an unblocked endpoint is refused by the pin as an invalid-field validation failure rather than by the veto as an unauthorized one. A sanction reads outward as a malformed request, and the security test that should prove the veto proves the pin instead.

   **It is a standing dependency on a pin, which is the reason to close it rather than bless it.** The guarantee holds only while every endpoint field stays pinned. This is not the accident §APR7.5.1 rule 4 describes — those eight pins carry their own stated reason, that an endpoint cannot change and its content type is an authorization input — but it is still one rule carrying another's weight, and §SEC14.6 rule 1's whole point is that a gate must be safe on its own terms. Closing it is small and local: leave the row-free half where it is and move the endpoint half below the storage read, composing it from the stored row exactly as remove and hard remove already do. **It is a foundation change and belongs to its own issue** — the behaviour predates the association orchestration's CRUD surface and no orchestration member changes.

   **§SEC14.6 is satisfied rather than waived.** Rule 1 asks every service to gate its own callers, and the orchestration gates every caller it can decide on the evidence it holds; rule 3 allocates the rest, each layer enforcing the rules appropriate to its altitude, and a rule composed "using only the columns on the row" is the foundation's altitude by that rule's own words. Rule 2 makes a duplicate **intended** where a layer can run one; it does not oblige a layer to buy the evidence for one. Either service is still safe when called alone — the foundation holds the row, and nothing reaches it through the orchestration ungated.
...
7. **A personal association is visible only to its owner and the review tier, on both read paths — reaction authorship is not published.** A row with `UserId` set (§DOM4.2) is admitted to its owner — **the account its `UserId` names, and not its `CreatedBy`** — and to the review tier of rule 2, and to nobody else: not to an anonymous caller, and not to an authenticated caller who is neither, **whatever its approval and publication state**. To everyone else a reaction exists only as §ARC16.8's aggregate. For that one kind of row it changes posture A rule 4 in **two** ways: it **narrows** its *public*, and it **moves its owner** from `CreatedBy` to `UserId` — which, on a row whose two columns differ, also **shows** a non-public row to the account `UserId` names where posture A rule 4 would have hidden it. It changes nothing for an editorial row, whose `CreatedBy` this rule does not decide. It decides **visibility** only: the owner test posture A rule 3 applies to removal is unchanged by it.
   ...
   **Why `UserId` and not `CreatedBy`.** Every rule that is specific to a personal row keys it on `UserId`. §DOM4.10 rule 1 derives it from the envelope, rule 5 makes `UserId != null` the one chain that decides the row is personal, and rule 7 revives a withdrawn row only when *"`DeletedBy == Association.UserId`"*. §DOM4.6 rule 2's personal index makes the reader's one row for a host unique on its endpoints together with `UserId`. Posture A′ rule 2 admits the reaction repoint only for `UpsertPersonalAssociationAsync` *"acting for that same `UserId`"*, and §ARC16.8's viewer fields name the caller's own reaction, pair-keyed on the caller. A visibility rule keyed on `CreatedBy` would be the one personal-row rule that did not. It would also reopen a probe §ARC16.8 closes: the withdrawal locates the caller's row by `UserId` (§ARC16.8.1), so under `CreatedBy` keying it could locate a row the caller cannot read and then be refused, and a refusal is distinguishable from the `204` the withdrawal otherwise answers — which tells the caller that a row they may not see exists. And the two columns **fail in opposite directions** if they differ: keyed on `UserId`, the row stays with the account it names; keyed on `CreatedBy`, it would be shown to whoever wrote it and hidden from the person whose reaction it is. **The owner tests on writes are not personal-row rules and are not changed by this**: modify, removal, sort and the set-confidence exclusion all test `CreatedBy`, which §DOM4.9 calls *"the row-local owner"*.

   **The consequence, and whether it can arise.** Where the two columns differ, the account `UserId` names can **read** the row and **cannot withdraw** it: the withdrawal resolves the row by `UserId` and then calls the foundation's soft delete, whose owner test is posture A rule 3's `CreatedBy`, which this rule leaves unchanged. The account `CreatedBy` names can remove it by id and cannot read it. **That case cannot arise on any designed write.** Every designed write stamps `CreatedBy` with the envelope's caller — the foundation's add refuses any other value — and derives `UserId` from that same envelope (§DOM4.10 rule 1), so the two agree whenever a service writes the row; and no caller-facing path writes a personal row today, because `AssociationOrchestrationService` sets `UserId` to null on every add until #188 derives it. The columns can diverge by two routes only, and both are named. `Association-Adding` still binds the foundation, which checks `UserId` for length only, so a row published on that address could carry a `UserId` naming someone other than its writer; nothing in the host publishes it, and **#631** closes it by moving the address onto the orchestration, where `UserId` is set rather than accepted. And rule 6's *"migration, backfill or direct SQL"*, which no service rule can bind. **So the gap is bounded, and owned where it can be.** Moving removal's owner test to `UserId` is **not** done here: under every designed write it would change no answer, and a divergent row is closed at the write, by #631, rather than accommodated at the read.
