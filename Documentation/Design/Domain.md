...
4. **Reclassification is forbidden.** `Type`, `KeyId` and `GroupId` are pinned against storage on every modify. Repointing an association is indistinguishable from deleting one link and creating another — except that it carries the original's approval state and review history across to a pair nobody reviewed.
   ...
   **The hazard this rule names is answered by mechanism rather than waived**, and both halves are closed.
   ...
   - **Approval state.** The repoint **leaves `ApprovalStatus` alone**. Whether approved content that changed must be re-approved is a question the design already owns generically: **`RequireReapprovalOnChange`** (§APR8.4), consulted by `ProcessEntityModifiedAsync`, whose house default is `true` (`ApprovalSettingSeedData.cs:65`) and which the seed sets **`false`** for `(Association, IsPersonal = true)` (`:209`). A reset written into the operation would be a second answer to a question the policy already answers — the two-sources-of-truth failure this design refuses everywhere else. **The policy decides:** under the seeded tier the row stays `Approved` and the reader's repointed reaction is counted without interruption; under a tightened tier the existing flow re-opens the round and dismisses active reviews on the new pair.
   - **Review history.** The repoint publishes **`Association-Repointed`**; `ApprovalOrchestrationService` subscribes to that fact and the ear delegates to the existing `ProcessEntityModifiedAsync` (§ARC16.2.2) — the same flow it runs for a content change — so the round re-opens and active reviews on the new pair are dismissed wherever the tier's `RequireReapprovalOnChange` asks for it. Under the seeded `(Association, IsPersonal = true)` tier there are none to dismiss.
