...
2. **It makes uniqueness a database guarantee.** Two `AllVersions` rows for the same group differing only in `KeyId` mean the same thing; over the raw columns they are distinct rows, and the effective id collapses them. This matters because foundation services are reachable through public event addresses and cannot assume an orchestration's retrieve-or-add ran first.
   ...
   **There are TWO unique indexes over it, not one.** The single `UX_Associations_Pair` — unique over `(EntityAType, EntityAEffectiveId, EntityBType, EntityBEffectiveId, UserId)` filtered `IsDeleted = 0` — is **replaced by the pair below**, because one index cannot key on different column sets for personal and editorial rows, and the two are genuinely different rules: *exactly one editorial row per pair*, and *exactly one personal row per (host, far-end type, reader)* — which is what makes a reader's single reaction row (§DOM4.10) a storage fact rather than a convention. **This rule owns the text; #627 applies it to the schema and cites this rule.**
   ...
   1. **Both filters are written EXPLICITLY.** Left to itself, EF filters a unique index over a nullable column with `WHERE [UserId] IS NOT NULL`, *"which would exempt every editorial row from the uniqueness it most needs"* — the reason already recorded in code at `StorageBroker.Association.Configurations.cs:258-274`, and it must survive the reshape rather than be rediscovered after it.
...
3. **`Purpose` joins `UX_Associations_EditorialPair` and both retrieve-or-add probes** (§DOM4.6 rule 2, which replaces the single `UX_Associations_Pair` with an editorial and a personal index and puts `Purpose` in the editorial one alone). The editorial pair index is deduplication — so the same image may be a header candidate *and* a gallery member of one host as two rows, and a header add cannot false-positive against an existing gallery row of the same pair.
