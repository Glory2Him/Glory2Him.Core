...
4. **Reclassification is forbidden.** `Type`, `KeyId` and `GroupId` are pinned against storage on every modify. Repointing an association is indistinguishable from deleting one link and creating another — except that it carries the original's approval state and review history across to a pair nobody reviewed.
   ...
   **The hazard this rule names is answered by mechanism rather than waived.** The approval-state half is closed; the review-history half is closed wherever `RequireReapprovalOnChange` is `true`, and under `false` the old pair's reviews count for the new pair by configuration (§APR8.6's first residual).

   - **Approval state.** The repoint **leaves `ApprovalStatus` alone**. The change goes through the **same approval process as every other modification**, ruled on #685 (2026-09-24): *"We are not inventing a different process. The same process apply for everything. On modification the modified event will trigger the approval process and the item will be auto approved just like before. Same work for the other entities. They are just versioned so they create a fork, We dont need / have a fork here"*. A versioned entity's change becomes a new row the process decides; a single-row one has no new row, so `ProcessEntityModifiedAsync` returns the row's own round, when decided — and with it the row (§APR9.8) — to `Submitted`, the status a reaction is created at (§ARC16.8.1), and evaluates it with the tier's rules. **`RequireReapprovalOnChange` keeps its meaning** (§ARC12.5.3 business rule 12): it decides whether the reviews the old pair earned are dismissed first, not whether the process runs. Its house default is `true` (`ApprovalSettingSeedData.cs:65`) and the seed sets it **`false`** for `(Association, IsPersonal = true)` (`:209`), a tier whose process auto-approves the change just as it approved the reaction when given. A reset written into the operation would still be a second answer to a question the process already answers. **Not yet built, and #618's:** today the flow leaves a decided round where it is and, under a `true` setting, dismisses its reviews and returns its AI reviewer's assignment to pending anyway.
   - **Review history.** The repoint publishes **`Association-Repointed`**; `ApprovalOrchestrationService` subscribes to that fact and the ear delegates to the existing `ProcessEntityModifiedAsync` (§ARC16.2.2) — the same flow it runs for a content change, which returns a decided round to `Submitted` before anything else (the bullet above), so a dismissal the tier asks for is always followed by the evaluation of the change. Under the seeded `(Association, IsPersonal = true)` tier there are none to dismiss.
