...
7. **`Approved` and `Rejected` are terminal.** A content item in either state is immutable in place — to its owner, to a publisher, and to an administrator alike. No role amends a terminal row's content, and there is no in-place exception (rule 16). Content items always fork, so for them this has no exception; where the design applies this rule to a single-row type it has one counterpart, §APR7.5.1 rule 3's change, which goes back through the approval process (#685, not yet built).
...
4. **Normalisation runs inside the foundation's own write path, before the storage call** — `DoAddAssociationAsync` today (`AssociationService.cs:427`), and `DoUpsertPersonalAssociationAsync` when §ARC16.2.2 is built — **not in the public method and not in an orchestration.** **Both named members are private write bodies**: this service's write shape is a public member that refuses a null argument and mints the envelope, delegating to a private `Do`-member that takes the envelope and does the work, and that shape does not depend on a second entry path — `SortAssociationAsync` (`AssociationService.Transitions.cs:80`) delegates to `DoSortAssociationAsync` (`:269`), which no substrate handler calls. Two reasons hold the placement there and only one is about the entry path. **Independent of the path:** `CK_Association_CanonicalOrder` refuses a non-canonical row by any layer and on any entry path (§DOM4.6 rule 2), so the row must reach canonical form in the code that writes it rather than in a layer the caller may not have come through. **On the add path only:** the foundation's add is reached without passing the orchestration by every caller that holds `IAssociationService` — `AddAssociationAsync` and `OnAddingAssociationAsync` are foundation members, and the latter is safe called alone (§SEC14.6 rule 1) — so anything layered above `DoAdd` is bypassed by those callers. Since #631 the `Association-Adding` address binds `IAssociationOrchestrationService` rather than entering `DoAdd` directly, but that handler ends by handing the same envelope to `OnAddingAssociationAsync`, so every add still reaches `DoAdd` whichever door it came through, and `DoAdd` remains the one place all of them pass. **`Association-Upserting` creates no matching bypass** — §ARC16.2.2 binds it to `IAssociationOrchestrationService` and never the foundation, and it is not minted under #618 — so the upsert's placement rests on the path-independent reason and the write shape alone.
...
4. **Reclassification is forbidden.** `Type`, `KeyId` and `GroupId` are pinned against storage on every modify. Repointing an association is indistinguishable from deleting one link and creating another — except that it carries the original's approval state and review history across to a pair nobody reviewed.
   ...
   **One narrow, structurally-constrained exception exists, and the rule is AMENDED rather than waived** — the same form §DOM4.1 already uses for `Purpose`. **`EntityBKeyId` and `EntityBGroupId` may be repointed on a PERSONAL association** — one whose `UserId` is set (§DOM4.2) — **and only by the dedicated `UpsertPersonalAssociationAsync` of §DOM4.10 and §ARC16.2.2, and only when `EntityBType` is unchanged.** `EntityAType`, `EntityAKeyId` and `EntityAGroupId` stay pinned, and so do **both endpoints' types**. **The general modify keeps refusing all of it**: the pin in `ValidateAgainstStorageAssociationOnModify` (`AssociationService.Validations.cs:622`) is unchanged, so the exception cannot be reached through `ModifyAssociationAsync`, and an **editorial** row is never repointed by any path.

   **The hazard this rule names is answered by mechanism rather than waived.** Both halves are closed: the change goes back through the approval process, in a round that starts with no reviews, so the old pair's reviews never count for the new pair (§APR8.8 regardless-rule 1, not yet built).

   - **Approval state.** The repoint **leaves `ApprovalStatus` alone**. The change goes through the **same approval process as every other modification**, ruled on #685 (2026-09-24): *"We are not inventing a different process. The same process apply for everything. On modification the modified event will trigger the approval process and the item will be auto approved just like before. Same work for the other entities. They are just versioned so they create a fork, We dont need / have a fork here"*. A versioned entity's change becomes a new row the process decides; a single-row one has no new row, so `ProcessEntityModifiedAsync` returns the row's own round, when decided — and with it the row (§APR9.8) — to `Submitted`, the status a reaction is created at (§ARC16.8.1), and evaluates it with the tier's rules. **`RequireReapprovalOnChange` does not govern this change** (§ARC12.5.3 business rule 12, ruled on #685): the round starts with no reviews, as a fork's does, so the reviews the old pair earned are dismissed whatever it says. The seed sets it `false` for `(Association, IsPersonal = true)` (`ApprovalSettingSeedData.cs:209`), a tier whose process auto-approves the change just as it approved the reaction when given. A reset written into the operation would still be a second answer to a question the process already answers. **Not yet built, and #618's:** today the flow leaves a decided round where it is and, under a `true` setting, dismisses its reviews and returns its AI reviewer's assignment to pending anyway.
   - **Review history.** The repoint publishes **`Association-Repointed`**; `ApprovalOrchestrationService` subscribes to that fact and the ear runs the flow `ProcessEntityModifiedAsync` runs, passing it the change's signed `UpdatedWhen` (§ARC16.2.2, §APR9.7.4) — the same flow it runs for a content change, which returns a decided round to `Submitted` before anything else and dismisses the old pair's reviews (the bullet above), so the dismissal is always followed by the evaluation of the change. Under the seeded `(Association, IsPersonal = true)` tier there are none to dismiss.

   **That subscription does not exist on the day this rule lands, and the rule says so rather than claiming a safety it does not yet have.** `ApprovalOrchestrationService` subscribes to exactly three `Association` facts — `Association-Added`, `-Modified` and `-Submitted` (`EventSubscriptionRegistration.cs:2074`, `:2091`, `:2222`) — and `Association-Repointed` is not among the seventeen `Association` addresses that exist at all (`EventBrokerIdentifiers.Association.cs:122-138`). **#618 builds the ear**, inside its criterion 8. This is §DOM4.9 rule 8's form (`Domain.md:525`), which records the `Attachment` arm as unblocked by a service that does not exist yet: a route is never named without the issue that builds it. This rule is the design's one anti-laundering rule, and a false safety claim is more expensive here than anywhere else.
...
2. **It makes uniqueness a database guarantee.** Two `AllVersions` rows for the same group differing only in `KeyId` mean the same thing; over the raw columns they are distinct rows, and the effective id collapses them. This matters because foundation services are reachable through public event addresses and cannot assume an orchestration's retrieve-or-add ran first.
   ...
   **There are TWO unique indexes over it, not one.** The single pair index they replaced — unique over `(EntityAType, EntityAEffectiveId, EntityBType, EntityBEffectiveId, UserId)` filtered `IsDeleted = 0`, created by the applied migration `20260806225353_AddAssociationPairUniquenessAndCanonicalOrder` and dropped by #627 — was **replaced by the pair below**, because one index cannot key on different column sets for personal and editorial rows, and the two are genuinely different rules: *exactly one editorial row per pair*, and *exactly one personal row per (host, far-end type, reader)* — which is what makes a reader's single reaction row (§DOM4.10) a storage fact rather than a convention. **This rule owns the text; #627 applies it to the schema and cites this rule.**

   | Index | Key | Filter |
   | --- | --- | --- |
   | `UX_Associations_EditorialPair` | `EntityAType, EntityAEffectiveId, EntityBType, EntityBEffectiveId` | `IsDeleted = 0 AND UserId IS NULL` |
   | `UX_Associations_PersonalPair` | `EntityAType, EntityAEffectiveId, EntityBType, UserId` | `IsDeleted = 0 AND UserId IS NOT NULL` |

   Five statements about that pair are load-bearing rather than incidental, and each is a ruling rather than a table entry.

   1. **Both filters are written EXPLICITLY, because neither can be left to EF.** Left to itself, EF filters a unique index on its nullable key columns alone — `[col] IS NOT NULL` — and never adds an `IsDeleted` term. **Editorial:** every key column is non-nullable, so EF would put **no** filter on it; the index would then take in every personal row, so two readers giving the same reaction to one item would collide, and a soft-deleted row would hold its pair for ever. **Personal:** `UserId` is its one nullable key column, so EF would filter on `WHERE [UserId] IS NOT NULL` alone, which drops `IsDeleted = 0` and leaves a withdrawn reaction blocking the reader from ever reacting again. The hand-written filter must keep its `UserId` term as well: SQL Server treats `NULL = NULL` as a duplicate in a unique index, so without that term every editorial row would enter the personal index and an item could carry only one editorial `Tag`. The code records the same reasons at `StorageBroker.Association.Configurations.cs:263-277`. **The single index's reason does not carry over.** It said EF's default would exempt editorial rows from the uniqueness they need. That was a hazard only while one index held editorial and personal rows alike. Here, keeping editorial rows out of the personal index is intended, and the editorial index has no nullable key column for EF to filter on. So neither index lets editorial rows escape the uniqueness they need, and the old reason must not be restored.
   2. **`UserId` LEAVES the editorial key.** Its filter pins the column to `NULL` there, so it carries no information and keying on it would be keying on a constant. What it did in the single index — collapse "exactly one of these globally" and "exactly one per user" into one constraint through SQL Server's treatment of `NULL = NULL` as a duplicate — is now done by the split itself, and `UserId` becomes the column that **routes a row to one index or the other** (§DOM4.10).
   3. **`EntityBType` STAYS in the personal key.** Dropping it would give *"one personal association of any kind per item"*, which forbids at the database level a future personal bookmark or rating coexisting with a reaction — a product decision nobody has taken, taken silently by an index. Keeping it gives *"one personal reaction per item"*, which is the rule that was actually stated.
   4. **`Purpose` (§DOM4.9 — designed, not built) joins the EDITORIAL key only**, giving `(EntityAType, EntityAEffectiveId, EntityBType, EntityBEffectiveId, Purpose)`, so the same pair may exist once per purpose. **The support for that disposition is §DOM4.9 rule 1 (`Domain.md:516`) together with §DOM4.10's `EntityTypePersonalisation`, and never this rule** — which said the opposite until it was rewritten, naming one combined key including `UserId`. `Purpose` is mandatory when an endpoint is an `Attachment` and forbidden when none is, and `EntityTypePersonalisation` marks every `EntityType` but `Reaction` non-personal, `Attachment` included; so an attachment association can never carry a `UserId`, and `Purpose` in the personal key would be a column that is always null in every row that index ever sees. The editorial index remains **deduplication**, not selection — it answers "has this exact statement been made before", while §DOM4.9 rule 5 answers "which candidate renders". Existing rows all carry `Purpose = NULL`, which a unique index treats as a value, so per-pair semantics on non-attachment associations are unchanged.
   5. **The personal key puts the HOST on endpoint `A`, and that rests on an ordinal fact which must be stated.** Ordinal order of the `EntityType` **names** — §DOM4.4 rule 1 orders on the name and never the numeric value — is `Association`, `Attachment`, `BibleReference`, `Comment`, `ContentItem`, `Link`, `Reaction`, `Tag`, while the eight members are *declared* `ContentItem = 0`, `Tag = 1`, `Reaction = 2`, `BibleReference = 3`, `Comment = 4`, `Link = 5`, `Attachment = 6`, `Association = 7`. The declared order and the name order are different orders, which is the whole reason this has to be written down. `(EntityAType, EntityAEffectiveId, EntityBType, UserId)` is right for a `ContentItem` paired with a `Reaction` because `"ContentItem" < "Reaction"`, **and only because of that**. A future personal far-end type whose name sorts *before* `ContentItem` — anything from `Association` through `Comment` — would land on **A** and put the host on **B**, and the index would then read *"one personal row per far-end entity"* rather than *"one per host"*. Nobody would have taken that decision; the index would have taken it silently. **Standing condition:** a new personal `EntityType` whose name sorts before `ContentItem` must not be admitted while this index keys the host on `A`. §DOM4.9 rule 6 (`Domain.md:523`) carries exactly this caveat for its own index, and is the form reused here.

      **The ordinal fact is ENFORCED, not merely stated.** `CK_Association_CanonicalOrder` compares `EntityAType` to `EntityBType` under `COLLATE Latin1_General_BIN2` — binary, therefore ordinal — so the endpoint pin is a **database check constraint and nothing is built for it**. Neither this rule nor anything downstream of it may describe the pin as resting on convention.

   Both indexes are paired with that same `CK_Association_CanonicalOrder`, without which the same pair written the other way round is a different key and the duplicate lands.

   **A probe that answers for an index before the database does must use the key of the index that governs the row. Today the set-scope pre-check does not.** Under §DOM4.10 rule 5, `IsPersonal` alone selects the governing index, so the set-scope pre-check (`ValidateAssociationPairIsUnoccupiedAsync`, through `IStorageBroker.ExistsLiveAssociationOnPairAsync`) must match the editorial key for an editorial row and the personal key for a personal one — the personal key has no `EntityBEffectiveId`. Today it matches the editorial key plus `UserId` for every row. For a personal row that is narrower than `UX_Associations_PersonalPair`, so a scope change onto a key where the same reader holds a *different* reaction passes the pre-check. The index then refuses the save, and the caller gets the generic dependency-validation error instead of the pre-check's refusal. The guarantee still holds; only the precise refusal is missing. **It is reachable today.** `Association-Adding` binds the foundation. Its handler enters `DoAddAssociationAsync`, which stores whatever `UserId` the caller sends and checks only its length. Only the orchestration forces the field to null. That entry closes when #631 moves the binding to the orchestration (§ARC16.2.2), not when #188 lands, and rows written through it before then remain.
   - **#618 owns the correction.** It writes the personal-key lookup that `UpsertPersonalAssociationAsync` needs (§ARC16.2.2), and the pre-check reuses that condition rather than writing a second one.
   - **The foundation service chooses the key.** It is never a branch inside the storage broker (§ARC12.2.1).
   - **The two retrieve-or-add probes stay as they are.** These are `AssociationService.Lookup.cs` and `AssociationService.OverlapLookup.cs`. They belong to the editorial arm (§ARC16.8.1) and must not be widened to the personal key. The personal arm finds its row inside `UpsertPersonalAssociationAsync` and never through them.

   **Open, not ruled: whether set-scope may change a personal row at all.** A reaction's host endpoint defaults to `AllVersions` (§DOM4.5 rule 1). If set-scope narrows it to `ThisVersionOnly`, the row moves to a different `EntityAEffectiveId`. The reader's next reaction on that item then misses the row in the personal lookup and creates a second live row. That breaks §DOM4.10 rule 6 without any index noticing. If set-scope is ruled to refuse personal rows, that refusal replaces the personal half of the pre-check correction above.
...
3. **`Purpose` joins `UX_Associations_EditorialPair` and both retrieve-or-add probes** (§DOM4.6 rule 2, which replaced the single pair index with an editorial and a personal index and puts `Purpose` in the editorial one alone). The editorial pair index is deduplication — so the same image may be a header candidate *and* a gallery member of one host as two rows, and a header add cannot false-positive against an existing gallery row of the same pair.
...
5. **One chain, and no second source.** The lookup decides whether `UserId` is set; `UserId != null` decides `IsPersonal`; `IsPersonal` selects the §APR8.4 approval tier (§DOM4.8, `Domain.md:503`) **and** selects which of §DOM4.6 rule 2's two indexes governs the row. Nothing else decides any of the three.
...
8. **A revive to the same reaction PRESERVES the `ApprovalStatus` the row was withdrawn at, and writes no status of its own.** There is nothing to restore, because the removal never changed one: §APR9.7.6 rules that *"a takedown leaves the approval record and the entity's denormalised `ApprovalStatus` alone"* (`Approval.md:709`) and that a restored entity *"resumes at its stored status with its review history intact"* (`Approval.md:1101`). So under the seeded `(Association, IsPersonal = true)` tier the row comes back **`Approved`** over a round already `Approved`, and it is counted again the moment it returns, with no fact having to be heard by anybody. **It is not restored to `Submitted`, and not to any status at all.** Forcing one would put a `Submitted` entity under an `Approved` round that nothing can reconcile — nothing subscribes to `Association-Restored` — which is the §APR9.8 divergence (`Approval.md:1131`) made permanent, and the reader's count lost for good.
...
- **Change from Love to Moved** — the same row is repointed and goes back through the approval process (§DOM4.5 rule 4); under the seeded tier it is auto-approved, so Love's count falls by one and Moved's rises by one in the same act.
- **Withdraw Love, then give Moved** — rule 6 allows no second row, so the withdrawn row is revived **and** repointed in one write. That is a change, not a revive of the same pair: it publishes `Association-Repointed` (§ARC16.2.2) and goes back through the approval process the same way.
...
2. Where both endpoints resolve a switch — a `BibleReference` ↔ `BibleReference` related-passage link resolves `RelatedBibleReferencesAllowed` on each end — the association is permitted only when **both** allow it. Denials union restrictively, matching the read-only role veto in §SEC18.6 rule 2.
...
3. A slug is always generated from `Title` — never accepted from a caller (§ARC12.4.1 business rules 6 and 12). Generation: lowercase, ASCII-fold, non-alphanumerics to hyphens, collapse and trim; on collision, suffix `-2`, `-3`, and so on.
