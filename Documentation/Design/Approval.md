...
**A changed reaction's return and dismissal are deltas, so they carry the redelivery check §EVN20 rule 4 requires** (#618, not yet built). It is §EVN19 rule 4's second shape, gating on signed state: the change's time is the `UpdatedWhen` on the fact's signed content (§EVN10), and the old pair's reviews are the round's reviews whose `CreatedWhen` precedes it. A decided round is returned to `Submitted` when it was decided before the change — its `UpdatedWhen` precedes the change's — or when one of the old pair's reviews still stands, because a round decided after the change while an old-pair review stood may have counted it, and nothing records which reviews a decision counted. The old pair's reviews that still stand are then dismissed, and the round is evaluated. Once the change has been processed, those reviews are dismissed and the round's `UpdatedWhen` follows the change, so a redelivered fact finds nothing left to do, and the evaluation it runs is the re-evaluation of the first shape.

**A round decided in that window is returned whatever decided it.** The window runs from the change to the moment the fact is first heard. An approval reached in it on the old pair's reviews, as when a new review lands before the fact is heard, must not stand. A direct decision taken in it is taken again, which fails closed. A failed first delivery only widens the window, so the retry closes it the same way.
