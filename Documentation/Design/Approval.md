...
**A changed reaction's return and dismissal are deltas, so they carry the redelivery check §EVN20 rule 4 requires** (#618, not yet built). It is §EVN19 rule 4's second shape, gating on signed state: the change's time is the `UpdatedWhen` on the fact's signed content (§EVN10). Only reviews whose `CreatedWhen` precedes it are the old pair's and dismissed, and only a decided round whose `UpdatedWhen` precedes it is returned to `Submitted`, so a redelivered fact finds nothing left to do once the change has been processed, and the evaluation it runs is the re-evaluation of the first shape.
...
**Two residuals, recorded here so that #618 builds against them knowingly.**

1. **A failed first delivery.** If the first delivery of `Association-Repointed` fails and the round is decided before the retry, the retry finds a round decided after the change and does not return it. The old pair's reviews still precede the change, though, so it dismisses them, and the round stays decided without the reviews that decided it. This is not closed here.
2. **The `-Modified` dismissal has no check.** Under `RequireReapprovalOnChange = true` the flow dismisses every active review, and returns Berean to pending, on every `-Modified` it receives (`ApprovalOrchestrationService.Flows.cs:90-143`). So a redelivered `-Modified` dismisses the reviews already cast on the amended text. This predates #685 and is not fixed by it; §EVN19 rule 4 and §EVN20 rule 4 name it.
