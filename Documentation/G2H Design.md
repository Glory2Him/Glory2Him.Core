...
| `GET` | `/api/Approvals/ReviewerDisplayNames?userIds=&userIds=` | What the given account ids are called — the §16.7.4 name resolver; requesting tier only. Not keyed on a round, because reviewers, invited people and candidates are all just ids a surface is holding. No role or disabled filter, so a reviewer who has since left the tier still has a name. Ids naming nobody are absent, not an error; a batch over 200 is a `400` rather than a truncated `200`. |
