...
**Most of this section is still a design proposal — the parts that are not are marked BUILT below.** Track A (#474, merged) built the settings, the offer, and a real, persisted assignment; it deliberately stops short of Berean actually reading anything or writing a comment, because the classification library that would produce one does not exist in this codebase (§13.4: "no AI broker or content-analysis service exists in code today"). Treat every "would"/"is proposed to" below as still exactly that, and every sentence marked BUILT as a hard rule, the same as §8.6/§8.6.1.
...
**Settings and gating — three booleans, and the second and third are both children of the first. The first two are BUILT; the third is BUILT as a stored, resolvable setting and DESIGNED, NOT BUILT as a behaviour.**
...
3. **`IsAIReviewerAutomaticallyRequested`** requires the first too, and governs one thing: whether a round that opens at `Submitted` assigns Berean **without anybody clicking** (§8.6.2.1). **BUILT (#531) as far as storing and resolving it goes** — the column, its migration and its backfill, `ApprovalSettingSeedData`'s constant and its drift comparison, the setting screen's switch, and the composition of the resolved value into `AIReviewerPolicyVerdict.IsAutomaticallyRequested` inside `AccessService.ResolveAIReviewerPolicyAsync` (§8.6.1 rule 4). **DESIGNED, NOT BUILT as a behaviour** — nothing consumes that verdict: no subscription, no seam verb, no unfiltered presence check and no automatic assignment (§8.6.2.1). An administrator can set it today and nothing acts on it. It is a sibling of the vote switch rather than a parent of it: it decides *how the assignment arrives*, never what Berean may do once it has one, so the two compose freely and neither reads the other.
...
### 8.6.2.1 Automatic assignment — DESIGNED, NOT BUILT
...
**What is built below, and what is not.** The setting this section reads is BUILT (#531): the column and its migration, the backfill, the seed constant and its drift entry, the admin switch, and the resolved value composed into `AIReviewerPolicyVerdict.IsAutomaticallyRequested` — which is gate 3's answer, already available to whatever asks. Nothing else below is built: the two subscriptions, the seam verb, the unfiltered storage-broker read gate 5 needs, the five gates themselves and the assignment they produce are all still proposal. The switch is therefore settable and answerable today and no round is assigned automatically because of it.
...
3. **The presence check of gate 5 must be UNFILTERED, and that needs a new storage-broker member.** An identity-filtered or soft-delete-filtered read must never decide an invariant — a read that answers nothing because the row is hidden is not the same as a row that does not exist — and this is exactly the case where the difference decides the behaviour. The only existing by-approval read, `IStorageBroker.SelectAIReviewerAssignmentByApprovalIdAsync`, hard-filters `IsDeleted == false` inside the query, so a **withdrawn** assignment reads as absent and the automatic path would re-assign Berean to a round a moderator had just taken it off — silently, and on every subsequent `-Modified`.

   **RULE — add `ValueTask<IQueryable<AIReviewerAssignment>> SelectAllAIReviewerAssignmentsAsync(CancellationToken)` to `IStorageBroker`**, which is the member the other fourteen entities on that broker already carry and the one `AIReviewerAssignment` was given without. `AccessBroker` composes the `ApprovalId` predicate over it and awaits there, with **no `IsDeleted` term** — the predicate and the await both in the broker, which is where a narrow read belongs and where `AccessBroker`'s existing `SelectAll*`-plus-predicate gathers already sit. No schema change, no new index: `UX_AIReviewerAssignments_ApprovalId` is filtered and answers the live question, and this read deliberately asks the other one. The existing filtered read stays exactly as it is; it answers a different question and every current caller is asking that one.

   *An earlier draft of this paragraph asserted that the check "needs no new storage-broker member", citing a `SelectAllAIReviewerAssignmentsAsync` that does not exist. It is corrected here rather than quietly dropped, because the wrong version reads as a cheaper design and a reader who found it would have built gate 5 on the filtered read.*

**The five gates, in this order.** Each is cheap before the one after it, which is the order and not a coincidence:

1. **The round is open.** `envelope.Content.ApprovalStatus == Submitted`, read before any gather. A `Draft` round has not entered review and there is nothing yet to review; a decided one is closed. This is the same signed-status gate §EVN18(e) condition 4 requires, and it answers every publisher rather than an enumeration of call sites.
2. **Berean is offered** — the resolved `IsAIReviewerOffered` is `true`. This is where the pairing the schema deliberately does not constrain (§8.6.2) is enforced, and it is asked fresh on every assignment exactly as the caller-facing write asks it.
3. **Automatic assignment is configured** — the resolved `IsAIReviewerAutomaticallyRequested` is `true`. Gates 2 and 3 come back from **one** `IAccessBroker` verdict, which resolves §8.4's tiering in its one home (§8.6.1 rule 4); the verdict gains a second field rather than the orchestration gaining an `IApprovalSettingService`. **That field is BUILT (#531)** — `AIReviewerPolicyVerdict.IsAutomaticallyRequested`, composed as `IsAIReviewerOffered && IsAIReviewerAutomaticallyRequested` in `AccessService.ResolveAIReviewerPolicyAsync` and nowhere else, so a handler that reads it has already asked gate 2 as well. An unresolved round answers `null` and fails closed, as it already does.
4. **The subject is visible** — the entity behind `(EntityType, EntityId)` reads back and is not soft-deleted. A takedown leaves the approval record and the entity's denormalised `ApprovalStatus` alone (§9.7.6), so a taken-down row still looks open to every status-shaped gate; without this, a `-Modified` on such a round would set an AI pass running over content nobody may see. This is §16.7.2's repair gate and §12.5.4 business rule 2's, held to the same standard for the same reason.
5. **Nobody has decided about Berean on this round** — there is no `AIReviewerAssignment` row for the `ApprovalId` **at all**, live or soft-deleted, per the unfiltered read ruled above. A live row means Berean is already assigned and this is a redelivery or a second route; a soft-deleted row means a human withdrew it, and an automatic policy must not overturn a person's decision on the round in front of them. The moderator's route back is unchanged — `POST` asks again explicitly — and it is theirs to take.

Gate 5 is also what makes the reaction **terminate**, which matters more than it looks: see §EVN18's reviewer and AI subscription tables.

**Failure posture — it never faults the round.** An automatic assignment is bookkeeping that runs *because* a round opened, never something the round waits on, so a failure of this handler must leave the approval exactly as it is. Delivery failure is already contained by construction (§EVN23) and reported through `EventPublishResult`, and the same posture §7.9 rule 8's retirement takes governs here: log it and let the round proceed. What a missed assignment costs is that Berean has not been asked — recoverable by one click, by the person who would have clicked it before this feature existed. That asymmetry is why fail-quiet is right here and would be wrong for any of the five gates, each of which fails **closed**.

**The `ProcessedEvent` ruling — neither handler records one, and that is ruled rather than omitted.** It follows the tier rule, not this feature: a fact handler above the foundation writes and checks nothing in that table (`Documentation/Design/Events.md` §EVN19 rules 1 and 4). Both halves of the pair are storage-broker calls an orchestration may not hold (§12.5), there is no transaction here to commit them in because this service writes no row of its own — the seam owns its unit of work — and neither question the pair answers arises: nothing asks this handler for anything, and the fact its write publishes is the foundation's. What stands in its place is gate 1's signed status plus gate 5's unfiltered presence check, which together make a redelivery a no-op: the second delivery finds the row the first wrote and stands down.

Two deliveries **at once** are the case that argument does not cover, and the answer is the one #479 already ruled (§12.5.4 business rule 4): both may pass gate 5 before either commits, and the filtered unique index `UX_AIReviewerAssignments_ApprovalId` refuses the loser, whose dependency-validation failure is contained by the posture above. The end state is identical either way — one assignment on the round — so no concurrency token is added here, for the same reason none was added there.
...
- **Watch for:** gate 5 being built on the existing filtered read. It is the one mistake here that produces no error, no log and no failing test — only Berean quietly reappearing on rounds a moderator took it off.
...
The rule that a user with an active review may not also set the entity's `ApprovalStatus` — administrators excepted — is answered from the `ApprovalReview` rows the approval policy already reads (§8.6 regardless-rule 1).
...
9. Only the owner (`CreatedBy`) may modify a content item or its versions. A publisher or administrator may amend the text of a `Draft` or `Submitted` item during review (typos/grammar); their identity is then recorded on `UpdatedBy`. `CreatedBy` never changes on an update. **A reviewer may not amend it at all** — the review tier is out of the modify gate at every scope (§14.7 posture A.3), and the amendment window closes for the publisher tier the moment the item turns terminal (rule 10 below).
...
| 3 | `ApprovalOrchestrationService` | Orchestrates the approval **round** — submission, review decisions, policy outcomes, and the denormalized state on the owning entity. Reviewer coordination is entry 5's and the AI reviewer's own lifecycle is entry 4's; see §12.5.3's banner for the split. |
| 4 | `AIReviewerOrchestrationService` | **Built** (#474). Asking Berean, asking again, withdrawing it, and reporting where it stands (§8.6.2). Spans `Approval` and `AIReviewerAssignment`. Absent from this table until now, which was drift rather than a ruling. **DESIGNED, NOT BUILT — it gains two subscriptions and a second seam** (§8.6.2.1): `Approval-Added` and `Approval-Modified` for automatic assignment, and `IAIReviewerAssignmentWorkflowService` beside the caller-facing `IAIReviewerAssignmentService` it already holds. It still maps **two** exception families, `Approval*` and `AIReviewerAssignment*` — the seam is a second door on the foundation class and shares its `TryCatch`, so §16.7.1 finding 2 counts the pair once. The subscriptions bring `IEnvelopeIntegrityBroker` and **not** `IEventBroker`, for the reason §12.5.4 business rule 1 gives: both handlers verify an inbound envelope and cause their write through a seam that publishes for itself. |
| 5 | `ApprovalReviewerOrchestrationService` | **Built** (#521, completed by #522). Who takes part in a round — reviewer candidates, invitations, withdrawal, and name resolution. Spans `ApprovalReviewRequest`, `ApprovalComment` and `IdentityUser`, and reads the `Approval` foundation for the read-triggered repair alone. §7.9 rules 6 and 8 run here, as this service's own subscriptions on `ApprovalReview-Added` and `Approval-Modified` (§12.5.4 business rule 4); `ApprovalReviewersController` binds this service alone for the five reviewer routes (#523). **Four dependencies and four arms — an approved deviation, recorded in the register above.** |

> **Six entries were withdrawn from this table, and entries 4 and 5 above are NOT them.** `ApprovalReviewOrchestration`, `ApprovalCommentOrchestration`, `TagOrchestration`, `ReactionOrchestration`, `CommentOrchestration` and `BibleReferenceOrchestration` once held numbers 4–9 here and were each planned before the §12.1 rule was applied. The numbers were reused when `AIReviewerOrchestrationService` and `ApprovalReviewerOrchestrationService` took them; the withdrawals below stand, and none of the six named here is being reinstated under another name — in particular `ApprovalReviewerOrchestrationService` is not `ApprovalReviewOrchestration`, which was withdrawn for touching a single entity type. The reviewer orchestration touches four (§12.5.4). Every one touches a single entity type, so none needs anything above its foundation — `Tag`, `Reaction`, `Comment` and `BibleReference` are additionally Single-Row (§7.5.1) so they never fork, while `ApprovalReview` and `ApprovalComment` are not approvable entities at all; their surviving rules moved to §12.3.1, and their versioning rules were deleted. **Why they were deleted is not the same for all four**, and the single sentence that used to be here — "describing properties those types never had" — was true of three and false of the fourth. See §12.3.1's split, which §7.5.1 rule 1 already anticipated.
>
...
>
> **Entries 4 and 5 are entry 3's decomposition, not new features.** Entry 3 grew to seven service references over six foundation objects, against the two-to-three the Florance guidance allows, and §16.7.1 went on recording "three dependencies, not ten" while the constructor carried more than twice that. What follows §12.5.3 splits it along the seams the design already drew — §16.7.4 says in as many words that review requests "compose beside" the round rather than into it, and §8.6.2's caller-facing trio had already left for entry 4. Nothing here changes a rule about approval; it changes which service holds which rule.

**RULE — an orchestration holds brokers, and the Florance count does not count them.** Every orchestration in this solution holds `ILoggingBroker`, and every one that takes part in the event substrate holds `IEventEnvelopeBroker` and `IEnvelopeIntegrityBroker`, plus `IEventBroker` **where it publishes** — `ApprovalOrchestrationService` does, and `ApprovalReviewerOrchestrationService` after #522 does not, because its two subscriptions verify and then cause their write through a workflow seam that publishes for itself (§12.5.4 business rule 1); `ApprovalOrchestrationService`, `AIReviewerOrchestrationService` and `ApprovalReviewerOrchestrationService` additionally hold `IAccessBroker`, which is the whole reason a service spanning several entity types can ask a policy question without resolving policy itself (§12.3.1, §8.6.1 rule 4). The count §12.1 and the Florance guidance are about is the count of **exception families a service must catch and map** (§16.7.1), and a broker obliges none: it holds no logic to validate (§12.2), so it raises no family of its own and its raw failures are what the closing `catch (Exception)` is for. That is why brokers sit outside the count — the reason is the arm, not the layer, and a broker that ever did raise a family would count like any other contract.

> **Three instruction files stated this the other way, and all three are corrected in the same change as this rule.** The clause was written as the tail of the same-kind rule, whose real subject is that an orchestration must not mix processing services with foundation services. Read literally it forbids `ILoggingBroker`, which would leave an orchestration unable to log, and it forbids `IAccessBroker`, which would put §8.4's most-specific-wins back inside every service that asks a policy question — the exact duplication §8.6.1 rule 4 exists to prevent. Each now bars a **storage** broker, which is the boundary actually being protected: an orchestration reaching storage directly skips every layer beneath it at once.
>
...
| --- | --- | --- |
| `ApprovalReviewerOrchestrationService` | Four dependencies, four arms, fifteen blocks | The read-triggered repair (§9.7.2 rule 1) is documented behaviour serving precisely these operations, and dropping it would answer a moderator `404` on a picker they can do nothing about. Alternatives were considered and rejected. Four is that service's **ceiling**: a fifth dependency is the tripwire to split it again. Full argument in §12.5.4 business rule 1. |

**"`ApprovalReviewerOrchestrationService` has four" is NOT a justification.** It is the one case that was argued and approved, and its own entry says so. Somebody who finds that service and argues "four is fine, there is precedent" is answered on this page, without anybody having to argue with them. §12.5 entry 1's `AssociationOrchestrationService` is in the same position and is deliberately **not** in the register: it is recorded as breaking the guidance and has never been granted a deviation — recording a break is not accepting one.

*Filed here rather than in §16.7.1 on purpose, and it is a decision somebody will revisit. This rule block sits directly under the orchestration table, so it is what the author of a **new** orchestration reads, and it already hosts a general rule about the Florance count's scope (the broker rule above). §16.7.1 is titled for ONE service; putting a solution-wide protocol inside it would repeat the filing mistake this design already names — "this sentence said 'three dependencies, not ten' while the constructor held seven" — where a general claim sat in a service-specific section and went stale unread. §16.7.1's counting rule carries a pointer here instead, so a reader counting arms still meets the protocol.*
...
> **Its subject is the round, and everything below is about the round.** Three other concerns once sat on this service and two have already left: the AI reviewer's caller-facing lifecycle went to `AIReviewerOrchestrationService` (§8.6.2, §12.5 entry 4), and reviewer coordination goes to `ApprovalReviewerOrchestrationService` (§12.5.4). What stays is opening a round, evaluating it, deciding it, resetting it, and syncing the outcome onto the owning entity.
>
> **The test for what belongs here is §8.5, not the word "approval".** A rule that can move a §8.5 predicate — a review, a comment, a threshold, a status — belongs to the round. A rule that cannot is coordination, and §7.9's opening sentence already says the request rows "appear in **no** §8.5 condition — the verdict, the counts and the blocks never read it." That is the whole of why the reviewer half can leave without any rule changing.
...
5. Self-approval on the ordinary route is blocked when `ApprovalSetting.AllowSelfApproval = false`. It does not reach the bypass, which an `Administrators` may take over their own submission (§8.6 HR-2).
...
19. **It holds three ARM-BEARING service references and three exception arms: `IApprovalWorkflowService`, `IApprovalReviewWorkflowService`, `IAIReviewerAssignmentWorkflowService`** — `Approval*`, `ApprovalReview*` and `AIReviewerAssignment*`, twelve downstream catch blocks, at the Florance ceiling and not over it. A **fourth** seam used to sit in the constructor and cost no arm: `IApprovalReviewRequestWorkflowService`, held for §7.9 rules 6 and 8. **It has left with #522**, which moved both retirements to `ApprovalReviewerOrchestrationService`'s own subscriptions (§12.5.4 business rule 4), so this service now holds no reference to the request record at all. The count of three is unchanged by its departure, because it never cost an arm — the retirement verbs share `ApprovalReviewRequestService`'s own `TryCatch` with the caller-facing door — and it is recorded here so a reader diffing the constructor against an earlier count does not read the removal as a correction. **The `ApprovalReview*` arm was landed ahead of this split, sequenced deliberately first (issue #518) so the split moves a working arm instead of writing it under the refactor**; `DismissStaleApprovalReviewAsync`'s validation-shaped refusals now report as `ApprovalOrchestrationDependencyValidationException` rather than falling to the catch-all. Two of those three are used here for a single verb apiece — dismiss a stale review, return Berean to pending — and they stay together because they are the same act on two records: at the two sites where a round's verdicts stop describing its content (§9.7.4's `RequireReapprovalOnChange` branch and §8.6 HR-4's reset), the human reviews and the AI assignment are brought into line as one step. Splitting them would put half of one decision in another service. §16.7.1 states the counting rule these three are measured by.

**The AI seam is designed to grow a second verb, and this service is not its caller.** §8.6.2.1 adds `AddAutomaticAIReviewerAssignmentAsync` to `IAIReviewerAssignmentWorkflowService`, called by `AIReviewerOrchestrationService`'s own subscriptions (§12.5 entry 4). This service goes on holding the seam for `ReturnStaleAIReviewerAssignmentToPendingAsync` alone, and its count of three is unchanged: a seam member this service never calls costs it no arm and no reference. The sentence above therefore says what **this service uses** rather than what the interface carries — an earlier form said the seam "carries one", which the second verb would have quietly falsified here and in §16.7.1 at the same time.

#### 12.5.4 ApprovalReviewerOrchestrationService

> **The service and its controller split are BUILT (#521, #522, #523).** Nothing in this subsection is a new rule. Every rule it names is §7.9's, §16.7.2's or §16.7.4's, already written and already built inside `ApprovalOrchestrationService`; what this subsection settles is that they live on their own service, and what that costs.
>
...
`ApprovalReviewerOrchestrationService` owns **who takes part in an approval round**: who is in scope for it, who has been asked, who answered, and what they are all called. It spans `ApprovalReviewRequest`, `ApprovalComment` and `IdentityUser`, and reads `Approval` for the read-triggered repair alone — which is §12.1 rule 2's definition of an orchestration several times over.

**Why this is a service and not a partial file.** It was a partial file — `ApprovalOrchestrationService.ReviewRequests.cs`, `.ReviewRequests.Validations.cs` and `.ReviewerDisplayNames.cs`, 1,108 of that service's 4,076 implementation lines — and the file split hid the thing that was wrong: one interface carrying two contracts, and a constructor at six foundation dependencies against the Florance two-to-three. §16.7.4's closing sentence had already ruled the coupling absent ("the request rows change no §8.5 outcome, so nothing here touches the evaluation, the verdict or the decision paths — the whole feature composes beside §16.7.1–§16.7.2 rather than into them"). A concern that composes beside another is a service beside it.

Responsibilities — the five caller-facing operations of §16.7.4 and §16.7.2's name resolver, moved verbatim:

1. `RetrieveReviewerCandidatesAsync(entityType, entityId)` — who is in scope for this round (§7.9 rule 3's eligibility, minus the owner and minus anyone a covering `ReadOnly` blocks).
2. `RequestApprovalReviewAsync(entityType, entityId, requestedUserId)` — §7.9 rules 1–4.
...
4. `WithdrawApprovalReviewRequestAsync(entityType, entityId, requestedUserId, deletionReason)` — §7.9 rule 5.
5. `RetrieveReviewerDisplayNamesAsync(entityType, entityId)` — the round's people, named once and composed once.
6. **The two retirements, reactively.** §7.9 rule 6 (an answered invitation retires itself) and rule 8 (a closed round retires what it never answered) live here with the rows they touch, and they are subscriptions rather than calls — see the business rules below.

Business Rules:

1. **FOUR service dependencies, FOUR exception arms, FIFTEEN downstream blocks: `IApprovalReviewRequestService`, `IApprovalCommentService`, `IIdentityUserService`, `IApprovalWorkflowService`** — plus `IAccessBroker` and the envelope and logging brokers. The **integrity broker arrives with business rule 4 (#522)**, and the event broker does NOT: business rule 4's two handlers verify an inbound envelope and cause their write through the workflow seam, which publishes for itself, so nothing on this contract publishes even after #522. An earlier version of this sentence expected both, which would have injected `IEventBroker` ahead of its first caller — the defect §16.7.1 already keeps against this design in the other direction, a constructor and a paragraph that disagree. Subscribing is not a reason either: `EventSubscriptionRegistration` holds the broker and binds the handler, exactly as it does for every other subscriber. **The workflow seam arrives with business rule 4 too, as a second constructor parameter, and it is not a fifth dependency** — §16.7.1 rule 2 counts two interfaces onto one class once where they share one family, which these do. `IApprovalReviewRequestService`'s workflow seam (`IApprovalReviewRequestWorkflowService`) is a second door onto the same class **and shares its `TryCatch`** — `ApprovalReviewRequestService.Transitions.cs` wraps both retirement verbs in the same one the caller-facing operations use — so both doors throw `ApprovalReviewRequest*` and cost one arm between them. That shared **family**, not the shared instance, is what keeps it to one arm; §16.7.1 states the rule and why the instance is the wrong thing to count. `ApprovalReviewRequest*`, `ApprovalComment*` and `Approval*` are four-block arms; `IdentityUser*` is three, per finding 1 — four + four + three + four = fifteen, not the sixteen a same-sized-arm assumption would predict.

   **`IApprovalWorkflowService` is here for the READ-TRIGGERED REPAIR and nothing else** (§9.7.2 rule 1, business rule 2). An earlier version of this rule said the service takes no such dependency and that this is what keeps it at three; that was retired by the ruling to keep the repair, and the count went with it.
...
2. **It resolves the round through `IAccessBroker.RetrieveApprovalReviewerScopeByEntityAsync`, and REPAIRS on a null rather than reporting one.** Every operation is keyed on an entity, and every one of them needs the round behind that key.
   ...
   **Both of the repair's gates travel intact.** The entity must be **visible** (§14.5 rule 3 — not merely present, because a taken-down row keeps its `ApprovalStatus` and this repository has no EF global query filters), and the entity must be **in play** at `Draft` or `Submitted` (§9.8 — opening a round at `Draft` beneath an `Approved` entity writes a divergence nothing would ever reconcile). The visibility gate is asked in the resolver as well, before the repair is reached, so the repair's own copy refuses nothing today; it is kept because that helper mints a row for an entity id that came straight off a route and must defend itself rather than trust its caller (§14.6 rule 2).

3. **`IApprovalCommentService` is a caller-facing read and must stay one.** The display-name resolver names the authors of the round's live comments, and §16.7.4 rules that they "arrive through `IApprovalCommentService`, so §14.7 posture D's own visibility filter decides which authors a given caller can name". Reading them off the gathering broker instead would name people whose words the reader cannot see. **It is not tradeable.** It is no longer also the argument that the count is three rather than two — this clause survives the repair ruling, the count it used to justify does not.
...
4. **Both retirements are subscriptions, not calls, and this is the one behavioural change in the split.** Rule 6 binds `ApprovalReview-Added`; rule 8 binds `Approval-Modified`. Neither needs a new address, and neither needs a fact that says *why* the round moved — but rule 8 does need two things the direct call had for free, and an earlier draft of this rule assumed the broker supplied both. It supplies neither.
   ...
   **Neither handler records an inbound `ProcessedEvent`, and that is ruled rather than omitted.** The dedup pair is a foundation request-handler device — both halves are storage-broker calls an orchestration may not hold (§12.5), and there is no transaction here to commit them in, because this service writes no row of its own. `Documentation/Design/Events.md` §EVN19 rule 1 carries the ruling and its three reasons; what stands in its place is 4(i)'s signed status gate plus the idempotent gather above.
   ...
   **§EVN18(e) is amended by this, and deliberately.** It records that `Approval-Added` and `Approval-Modified` have no workflow handler bound, on the reasoning that the *re-test* is driven by the entity's and the workflow records' facts. That reasoning is unchanged and the re-test still binds none of them. What (e) did not distinguish is a subscriber that is not re-testing, and rule 8's retirement is exactly that — it reads no §8.5 predicate, moves none, and decides nothing. See `Documentation/Design/Events.md` §EVN18(e) for the amended wording and the boundary it now draws.

5. **Every posture in §16.7.4 travels with the operations and none of them is re-derived here** — the requesting tier and nobody else; account id, display name and username and nothing else; the round-keyed set with no caller-supplied ids; the double gate (this service admits the tier, the foundation applies §14.7 posture D again beneath it, §14.6 rule 2). A reviewer checking this split should be able to diff the moved methods and find no behavioural change outside business rule 4 — which is two changes, not one: the trigger becomes a subscription, and the rule 8 gather gains the answered-reviewer exclusion 4(ii) requires. Both are in the broker and the handler, not in the moved methods.
...
| 11 | `AttachmentsController` *(future)* | Exposes endpoints for attachment management. Upload and the media read are host endpoints following the profile-image precedent (§5.6.2–§5.6.3, §17.6), not controller surfaces. |
| 12 | `ApprovalSettingsController` | Exposes endpoints for approval policy settings (§8.2, §8.4). **Built**, and absent from this table until now. |
| 13 | `AIReviewersController` | Exposes `GET`/`POST`/`DELETE api/AIReviewers/{entityType}/{entityId}` (§8.6.2). **Built** (#474), bound to `IAIReviewerOrchestrationService` alone, and absent from this table until now — the same drift §12.5's orchestration table carried for the service behind it. **Its own resource, not a sub-resource of the approval round**, which is the precedent §12.5.4 business rule 6 rests on: Berean's routes were new, so they took a new address, where the reviewer-coordination routes already exist and keep theirs. |
...
**That is the tier rule rather than a tally of services, and two further subscriber sets are designed on it.** Above the foundation nothing writes or checks `ProcessedEvents` at all — the reasons are `Documentation/Design/Events.md` §EVN19 rules 1 and 4's, and they are structural rather than per-service: an orchestration holds no storage broker to write the record with (§12.5), and it has no transaction of its own to commit it in. `ApprovalReviewerOrchestrationService`'s two subscriptions (§12.5.4 business rule 4) and `AIReviewerOrchestrationService`'s two (§8.6.2.1) therefore keep no record, and neither is exempt from anything by doing so. The first pair is **built** and its ruling already recorded; the second is **DESIGNED, NOT BUILT**. What stands in the record's place in each case is a signed status gate read before any gather plus a gather or presence check that finds nothing left to do on a second delivery. That is the same footing §EVN18(d) already rests on, stated once here so the list above is not read as an inventory somebody must extend per service.
...
3. **Modify: owner (`CreatedBy`) or the `Publishers` tier** — the global `Publishers` / `Administrators`, `%EntityType%-Publishers`, and on `ContentItem` `ContentItem-%ContentType%-Publishers`. **The `Reviewers` tier is not in it, at any scope.** A reviewer reviews: they cast approval reviews and write approval comments, and that is the whole of their authority over somebody else's row. Rewriting the text underneath the verdict they are about to cast is HR-3 on the other surface — and this gate used to permit exactly that while HR-3 refused them the status one field away. *(This replaces "owner or review role", which described the behaviour before this ruling.)* The review tier keeps every **read** it had: rule 4 and §14.5 still admit it to non-public rows and to audit, because a reviewer must be able to see the draft they are reviewing. Only the modify gate moves off the review role.

   **The non-owner branch is bounded by status, and the owner branch by §3.4 rule 16.** The publisher tier and `Administrators` may amend only while the row is `Draft` or `Submitted` — the in-flight window §18.6 already granted for `Submitted`, now widened to `Draft`. Once a row is **`Approved` or `Rejected` it is terminal to them**: no in-place amendment, whatever the role. It is terminal to its **owner** too — a `Versioned` entity forks a new version instead (§9.2, §10.17), a non-versioned one is refused outright and the only route back is the §8.6 HR-4 override. The status is read from the **stored** row, never from the caller's copy: a caller who could supply it would self-certify their way past the terminal bar.

   The `Draft` ↔ `Submitted` carve-out (§9.2 rules 4–6) is untouched by all of this. It is a status transition rather than an amendment, and it already runs through the owner or the publisher tier — which is now the same set the modify gate admits.
...
3. **The veto is scoped to writes — and the approval OUTCOME is one of them.** `Approval`, `ApprovalReview`, `ApprovalComment` and `ApprovalReviewRequest` have no role vocabulary of their own — there is no `Approval-Reviewers` — so their scope is derived from the attached entity, which for an association means both endpoints. A block in scope stops the holder **casting or changing a review, deciding the approval, and amending the approval record**, and it drops them from the reviewer candidates so nobody can invite a person who cannot answer (§7.9 rule 3, §16.7.4). It does **not** reach the comment thread's **words** — adding, amending and withdrawing a comment are stopped by the global `ReadOnly` alone; §18.6 rule 2 records why. It **does** reach `IsResolved`, which is the one comment field that moves a §8.5 gate: `ResolveApprovalCommentRequest` carries the subjects, and the veto is asked ahead of the author branch (§14.7 posture D rule 5).
...
3. `Approval`: add/modify/remove gate is the global contribution gate; modify by owner or review role (resubmission by the submitter, status transitions by reviewers); remove by owner or `Administrators`; hard remove `Administrators` only. **"Status transitions by reviewers" excludes the two outcome statuses.** Moving an approval *into* `Approved` or `Rejected` is applying the §8.6.1 decision, and that additionally requires the **`Publishers` tier** (HR-3: reviewing is vouching, deciding is deciding), asked through `IAccessBroker.MayDecideApprovalByIdAsync` on top of this gate. Everything else the gate admits stays open to the review tier and to the submitter — resubmitting, for instance — because no transition matrix constrains this verb beyond the outcome gate and the standing refusal of `Dismissed`; it is authorization, not a state machine, that narrows it. **Reopening a decided round is NOT among them.** This sentence previously named it, which contradicted §9.4 and §9.6 ("only an administrator re-opens a decided one") and would have let the submitter undo a rejection of their own work. Moving a round out of `Approved` or `Rejected` is the §8.6 HR-4 override and is `Administrators` alone, whether it is reached through the entity's transition verb or through `ResetApprovalAsync` (§16.7.5). This sentence previously read as though a reviewer could decide through the general modify, which was the behaviour before that gate existed (§9.7.5).
...
**RULE — a count over three needs an entry in §12.5's register of approved deviations.** That is where the protocol for granting one lives, beside the rule an author of a new orchestration reads; this section deliberately holds only the counting rule and the inventory it produces.
...
3. **Brokers stay outside the count because they oblige no arm**, not because of which layer they sit in. A broker raises no exception family of its own (§12.2 — it holds no logic to validate), so its raw failures are what the closing `catch (Exception)` is for. §12.5's broker rule states this the same way. A broker that ever did raise a family would count like any other contract.
...
**`RetrieveApprovalVerdictAsync` answers "what may happen to this approval now, and what is stopping it".** It grants nothing and decides nothing. It writes exactly one thing, and only in one case: **a missing round is opened**, per §9.7.2 rule 1's read-triggered case. Nothing else — no evaluation, no transition, no command, no bypass. It carries:
...
**The one write, and why it stops where it does.** Two ways leave an entity with no `Approval` at all — seed data written straight to the storage broker, which publishes no fact, and a fact that does not land — and every read here keys on the approval, so such an entity answers `NotFound` to a caller who can do nothing about it. The read therefore *opens* the round (§9.7.2) rather than reporting its absence.
...
#### 16.7.5 Resetting a decided round
...
**`Administrators` alone.** Deciding an open round is the `Publishers` tier's work; *un*deciding a closed one is the §8.6 HR-4 override, and the override has one holder — a state a publisher could edit out of would not be terminal at all. The panel hides the control from the publisher and review tiers, and the orchestration refuses them; §14.6 rule 2 makes that pair deliberate.

**§8.6 regardless-rule 1 does not apply.** It bars anyone holding an active review from *deciding* the round, and a reset decides nothing — it takes a decision away. An administrator who reviewed may still put their own round back.

**Every active review is dismissed**, regardless of `RequireReapprovalOnChange` (§12.5.3 rule 12). The reviews produced the verdict being overruled, so re-opening on their strength would let the override be undone by the very reviews it overrode. The status is written first and the dismissal follows, so that the re-test each dismissal triggers sees the round as it now stands rather than the outcome being taken away. §8.8 regardless-rule 1 *requires* dismissal to work on a terminal round an administrator has moved back, so nothing refuses it either way — the order is a correctness choice, not a workaround.

**The bypass pair is cleared.** It records how *this* decision was reached (§9.7.5), and after a reset there is no decision: a round back with the reviewers must not still claim a waiver for an outcome it no longer holds.

**It unpublishes, and that is the operation working.** The entity follows through the ordinary approval command, and a transition landing anywhere but `Approved` forces `IsPublished = false` and clears `PublishDate` (§9.7.1). Resetting an approved item therefore takes it off the public site until it is approved again, and for a versioned entity its group has nothing published meanwhile — the same recoverable dark state §9.7.7 rule 7 already describes. A reset that left the accident publicly visible would recover nothing. Because a rejected item was never public, only the approved case is worth warning about, and the panel confirms on that case alone.

**The workflow does not react to its own writes.** The reset commands the entity to `Submitted`, the entity's transition answers under the workflow identity and publishes `-Submitted`, and the approval orchestration subscribes to that address. Re-entering the flow there would re-evaluate the round it just re-opened: under any tier with `RequireApprovals = false` and `AutoApproveIfAllApprovalRequirementsMet = true` — the shape the seed writes for `(Association, IsPersonal = true)` — the conditions are trivially met and the round is driven straight back to `Approved` in the same request, undoing the override. So the substrate reaction refuses any entity fact carrying the system identity. A human submission carries a human identity and is unaffected, and the envelope is verified before the flag is read, so no caller can assert it (§14.6 rule 4).
...
**The candidates read is a user-enumeration surface, and that decides its posture.** Nothing else below the Administrators-only admin endpoints lists users, so this read is exposed to exactly the tier that may request (§7.9 rule 2) and returns the minimum a picker needs — account id, display name and **username** — and nothing else. The username was added to that minimum deliberately: a display name is not unique, two colleagues called "John" are ordinary, and a moderator picking between them from the name alone is guessing. It costs nothing a caller could mine that the display name did not already cost — a username is how a person is addressed throughout the site, and §18.3.1 forbids it from being an email address — while a role list, an email or an account state would, and none of those are here. Answering the request rows (`ApprovalReviewRequest` reads keyed by `ApprovalId`) follows the same tier: they are moderation coordination, not public state, so §14.7 posture D applies to them as it does to the verdict. **That read is now built** — `RetrieveApprovalReviewRequestsAsync` above — and it gates twice on purpose: the orchestration admits only the requesting tier, and the foundation applies posture D again beneath it, which §14.6 rule 2 makes a deliberate duplicate rather than a redundancy.
...
**One gather, three rules.** `IAccessBroker.RetrieveApprovalReviewerScopeByEntityAsync` — the by-id form's entity-keyed twin, and what the reviewer orchestration actually calls (§12.5.4 business rule 2) — returns the round's status (rule 7), the entity's owner (rule 3), the role subjects, the active reviewers, the recorded ones and the active requests in a single read. The last two are gathered without the caller-facing visibility filter, for two different reasons. The requests are unfiltered for the reason `FindDismissableApprovalReviewIdsAsync` already is: deciding invitability from a filtered view would tell a moderator somebody is invitable and then collide with the uniqueness index. The recorded reviewers are unfiltered for the resolver's sake and for nothing to do with invitability — a panel renders a dismissed review, so the round has to be able to name the person who wrote it.

The request rows change no §8.5 outcome, so nothing here touches the evaluation, the verdict or the decision paths — the whole feature composes beside §16.7.1–§16.7.2 rather than into them. **That sentence is what licenses §12.5.4**: a feature that composes beside another is a service beside it, and the only reason it was not one from the start is that it was built while §16.7.1's dependency count was still believed to read three.
