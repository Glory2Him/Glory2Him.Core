...
8. **A closed round retires what it never answered.** When an approval reaches an outcome — `Approved` or `Rejected` — every request still pending on it is soft-deleted under the system identity.
   ...
   **Rows on rounds closed before this rule existed are left alone, and they do not age out on their own.** No migration backfills them, and nothing else reaches them either: a reset retires nothing (above), and a re-versioned entity opens a *new* approval rather than touching the old round's rows. The only thing that clears one is a moderator withdrawing it by hand, or the round being reset and then decided again — which runs this rule over it. The count is small and bounded by how many invitations were outstanding when each round closed, which is why it is left rather than migrated.
...
4. **Both retirements are subscriptions, not calls, and this is the one behavioural change in the split.** Rule 6 binds `ApprovalReview-Added`; rule 8 binds `Approval-Modified`. Neither needs a new address, and neither needs a fact that says *why* the round moved — but rule 8 does need two things the direct call had for free, and an earlier draft of this rule assumed the broker supplied both. It supplies neither.
   ...
   **`FindRetirableApprovalReviewRequestIdsAsync` therefore excludes any `RequestedUserId` with a RECORDED review on the round** — recorded, not active, matching `ApprovalReviewerScope.RecordedReviewerUserIds`: a dismissed review still means the person answered, and rule 6 retired their invitation when they did. With the exclusion the ordering is irrelevant and neither subscription has to know the other exists.

   **What that trades, stated rather than discovered later.** If rule 6's own retirement fails, the answerer's row now survives the close instead of being swept with a false reason. That is the right way round: rule 8's own text says "the clean-up is about what the record *says*, not about what it gates", and it already accepts a stale row as its failure cost — clearable by hand. A row asserting the wrong sentence is not clearable by anything, because nothing can tell it from a true one.
