...
A comment may only be created against an **existing** approval record — that half is enforced by the foreign key, so the service needs no read of `Approval`. The **non-deleted** half is *not* enforced by the foreign key and cannot be: an `Approval` is soft-deleted (§10.4 — the row stays and `IsDeleted` is set), so the key is satisfied by a deleted parent. It needs either a gated boolean probe over the unfiltered store, following the §14.6 pattern §9.7.2 rule 3 already uses for approval resolution, or a decision that commenting on a closed approval is permitted. **Open — neither has been chosen, and the non-deleted half is unenforced today.**
