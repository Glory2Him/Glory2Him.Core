...
**Settings and gating — three booleans, and the second and third are both children of the first. The first two are BUILT; the third is DESIGNED, NOT BUILT.**
...
3. **`IsAIReviewerAutomaticallyRequested`** requires the first too, and governs one thing: whether a round that opens at `Submitted` assigns Berean **without anybody clicking** (§8.6.2.1). **DESIGNED, NOT BUILT** — the column, its migration and the setting screen's checkbox all land with the feature, and none of them exists today. It is a sibling of the vote switch rather than a parent of it: it decides *how the assignment arrives*, never what Berean may do once it has one, so the two compose freely and neither reads the other.
...
**Storage and migration shape.** One new column on `ApprovalSettings`:
