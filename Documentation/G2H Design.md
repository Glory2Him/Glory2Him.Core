...
1. **Three service dependencies, three exception arms: `IApprovalReviewRequestService`, `IApprovalCommentService`, `IIdentityUserService`** — plus `IAccessBroker` and the envelope, event, integrity and logging brokers. `IApprovalReviewRequestService`'s workflow seam (`IApprovalReviewRequestWorkflowService`) is a second door onto the same class **and shares its `TryCatch`** — `RetireApprovalReviewRequestAsync` wraps both retirement verbs in the same one the caller-facing operations use — so both doors throw `ApprovalReviewRequest*` and cost one arm between them. That shared family, not the shared instance, is what keeps the count at three; §16.7.1 states the rule and why the instance is the wrong thing to count.
   ...
   **Two of the three arms do not exist yet and are part of this work**: `ApprovalComment*` and `IdentityUser*` have no handler on the service these operations are moving off, so their validation failures currently reach the client as a 424 (§16.7.1's measured inventory). Moving the operations without writing the arms would move the defect.
...
> **Measured against that rule, THREE ARMS ARE MISSING TODAY, and the split must add them rather than inherit the gap.** `ApprovalOrchestrationService` holds six foundation objects and maps **three** families — `Approval*`, `ApprovalReviewRequest*`, `AIReviewerAssignment*`. `ApprovalReview*`, `ApprovalComment*` and `IdentityUser*` have no arm at all: `DismissStaleApprovalReviewAsync` propagates into the single `TryCatch<T>` with no local handler, and the comment resolver's and the candidates read's failures do the same, so all three land on `catch (Xeption downstreamException)` and reach the client as a 424 — precisely the fault the `ApprovalReviewRequest` arm was written to stop.
>
...
>
> **The post-split inventory is therefore THREE arms each, and each service must carry all three** — twelve blocks, since an arm is four. `ApprovalOrchestrationService`: `Approval*`, `ApprovalReview*` (**to add**), `AIReviewerAssignment*`, and `ApprovalReviewRequest*` **removed** with the dependency. `ApprovalReviewerOrchestrationService`: `ApprovalReviewRequest*` (moved), `ApprovalComment*` (**to add**) and `IdentityUser*` (**to add**). Twelve downstream blocks apiece, at the ceiling and not over it. A reviewer should check the arms before the constructor: the arms are what the rule is actually measuring.
