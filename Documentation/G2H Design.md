...
**One resolver, not a projection per read.** The panel needs names for reviewers, for invited people and for candidates; a display-name projection on the review read would have answered the first and left the next to invent its own, and three lookups are three chances to render one person under two names. One read answers all three instead, and every name is composed by one method — the same one the candidates read uses.

**It IS keyed on a round, and that is what makes its posture hold.** The set it names is built from the approval's own rows: the review rows — **dismissed and soft-deleted ones included**, because the panel renders those and their authors still need naming — the outstanding invitations, and the entity's review tier. That covers every surface the panel draws, which is why the unscoped form bought nothing the round-scoped one does not: all three belong to the SAME round. Nor did the bug force the unscoped form — a departed reviewer's id is still stamped on the review row, so a resolver scoped to this approval resolves them, which is exactly what the candidates read cannot do. What round-scoping buys is that the tier gate composes with an entity gate instead of standing alone: a `Tag-Reviewer` can name the people a tag round involves rather than any account id in `AspNetUsers`. It costs one extra read to establish the round, and the machinery for it already exists — `ApprovalReviewerScope` gathers it in the same shape the invitation operations use.
...
**It applies no role filter and no disabled filter over the round's own people, deliberately.** Their ids came off rows the approval already stores, so the account is part of the record whatever has happened to it since; filtering by the review tier is exactly the mistake that left a departed reviewer with a blank name. It is a user-enumeration surface all the same, so the posture above governs it rather than being re-derived: the requesting tier and nobody else, an account id and a display name and nothing else. The tier read is taken first and returns whole accounts, so the same rows that admit a candidate also carry the name and the ordinary round costs one identity read; a second one is spent only on the people the tier no longer holds. Ids naming no account come back absent rather than as an error, so one deleted account cannot blank a whole panel. The id echoed back is read off the resolved row, so it is always the canonical form, and a caller holding ids in another spelling normalises its own side to join on the answer.
...
| Every name it renders | `GET api/Approvals/{entityType}/{entityId}/ReviewerDisplayNames` — the §16.7.4 resolver, asked once for the round, which names its reviewers, its invited people and its candidates together |
