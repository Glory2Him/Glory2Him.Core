...
| 6 | `ApprovalReviewService` | Reviewer decision recording and row-local validation. Threshold evaluation, one-active-review enforcement and dismissed-review exclusion are explicitly **not** its responsibility (§16.5, §12.3.1). |
...
`Tag`, `Reaction`, `Comment`, `BibleReference`, `ApprovalReview` and `ApprovalComment` need **nothing above their foundation service**. Each touches one entity type, so under §12.1 rules 1–2 there is nothing to orchestrate. Beyond that the six split into two groups, and the reason differs:

- `Tag`, `Reaction`, `Comment` and `BibleReference` are approvable (§7.5) and **Single-Row** (§7.5.1), so there is no version fork, and §10.17 rule 1 permits the approval workflow to subscribe to their foundation facts directly.
- `ApprovalReview` and `ApprovalComment` are **not approvable entities at all**. Neither appears in §7.5, neither is an `EntityType`, and neither implements `IApproval` — they are the machinery of the approval workflow, not its subjects. §7.5.1 and §10.17 therefore do not apply to them in either direction: they have no publication model to look up (§7.5.1 rule 2 makes a missing row a hard error, not a Single-Row default), and the approval workflow does not subscribe to their facts. They qualify on the §12.1 entity-count rule alone.
...
1. Control fields are **every member of `IKey`, `IAudit` and `IApproval`** (§9.7.1 rule 2's subtraction test, which replaces per-entity enumeration and cannot fall behind a new interface member). That is `Id`; `CreatedBy`, `CreatedWhen`, `UpdatedBy`, `UpdatedWhen`, `IsDeleted`, `DeletedBy`, `DeletedWhen`, `DeletionReason`; and `ApprovalStatus`, `IsPublished`, `PublishDate`, `IsApprovedByBypass`, `ApprovedByBypassReason`. None is ever accepted from an external caller — each is set internally by the foundation or by the approval workflow (§8.6.1).
2. On every update the foundation loads the current entity from storage and maps only the permitted caller-supplied content fields onto it: `Name` (`Tag`), `Name` and `UnicodeEmoji` (`Reaction`), `Content` (`Comment`), `Reference`, `Translation` and `Scripture` (`BibleReference`). Every other field is **pinned against storage**, not merely omitted.
3. **The natural-key fields are create-only.** `Tag.Name`, `Reaction.Name` and `BibleReference.USFM` each sit behind a unique index and are the §7.5.1 rule 4 ground for these types being Single-Row at all. `USFM` is accepted on add and pinned against storage on every modify. Whether a `Tag` or `Reaction` rename is permitted against a live unique index is **open** — see #201, which covers the same indexes being unfiltered.
4. **An amendment of a terminal row (`Approved` or `Rejected`) is refused.** These are Single-Row, so there is nothing to fork into (§7.5.1 rule 3) — the row that is edited *is* the published row. The only route back is an `Admin` override to `Submitted`, which dismisses active reviews (§8.8 regardless-rule 1). Editing a `Draft` or `Submitted` row is the general modify of rule 2.
5. Soft delete follows §10.4. **Dependent associations are left untouched** — a soft delete breaks no link, and association visibility is the read-time composite of §14.3 rules 3–4, not a flag written on delete (the §12.4.1 responsibility 5 correction).
6. A soft-deleted row must not be visible on any content item.
7. **A soft-deleted row must also not be usable as an endpoint for a *new* association.** This is distinct from rule 6: rule 6 hides existing links, this one refuses new ones. It is enforced where associations are created, not here — `AssociationOrchestrationService` resolves each endpoint through its own service, and a visibility-filtered read reports a soft-deleted row as not-found.
8. Each publishes its own `-Added` / `-Modified` / `-Removed` facts on its own addresses (§10.2). `ApprovalOrchestrationService` subscribes to the `-Added` and `-Modified` facts **only — never `-Removed`** (§9.7.6: a removal is a takedown, not a moderation step), to manage approval records and workflow state. The foundation tier is the correct one to subscribe at for a Single-Row entity (§10.17 rule 1).
9. Review dismissal is not their responsibility. Publishing the entity's own `-Modified` fact is sufficient; `ApprovalOrchestrationService` handles dismissal when it receives that fact (§10.17).
10. State transitions (submit, approve, reject) are the foundation's own operations, each owning a narrower field scope than the general modify (§9.7.1 rule 3, §10.2 rule 7).
...
5. **The endpoint need not be `Approved` when the link is created.** The withdrawn wording said "associate an *approved* X"; that precondition is deliberately not carried forward, because approval is composed at **read** time by §14.3 rule 4, which already hides a link whose endpoint is not visible. Enforcing it at write time as well would block the ordinary flow of suggesting a tag that is still in review, and would still leave the link stale if the endpoint were rejected afterwards.
...
**`ApprovalReview`** — `ApprovalReviewService` owns the row-local half: the decision record itself, `ReviewerId` bound to the acting user on add and pinned against storage on modify, dismissal retention for audit (a dismissed review is never deleted), and a new review permitted after a previous one was dismissed. Reviewer eligibility is decided by `IAccessClient` behind `IAccessBroker`, not in the service — the reviewer must hold the review tier for the entity actually under review (§8.3, §8.6.1), and **must never review their own content: §8.6 HR-1 is unconditional, and `AllowSelfApproval` does not relax it** (that setting governs HR-2 approval, not review).

Three things §16.5 names as explicitly **not** this service's responsibility, because each needs `ApprovalSetting` or the whole review set that a single-entity foundation service may not read: enforcing one active review per reviewer (that is the unique index `UX_ApprovalReviews_ApprovalId_ReviewerId`, §7.7 rule 1, with the policy-level refusal in `IAccessClient`), evaluating approval thresholds, and excluding dismissed reviews from those calculations. Threshold evaluation belongs to `ApprovalOrchestrationService` (§12.5.3 R5) — which is what made a separate `ApprovalReview` orchestration redundant. §16.5 is the authority for this split; keep the two in step.

It publishes its own `-Added` / `-Modified` / `-Removed` facts on its own addresses (§10.2).

**`ApprovalComment`** — control fields `ApprovalId`, `UserId`, `IsDeleted`, `CreatedBy`, `CreatedWhen`, `UpdatedBy`, `UpdatedWhen`, `DeletedBy`, `DeletedWhen`, `DeletionReason`. On every update the service loads the stored comment and maps only `Comment` onto it; every other field is pinned against storage, not merely omitted. Soft delete follows §10.4. Approval comments do not participate in the threshold or status-transition workflow. It publishes its own `-Added` / `-Modified` / `-Removed` facts on its own addresses (§10.2).

A comment may only be created against an **existing** approval record — that half is enforced by the foreign key, so the service needs no read of `Approval`. The **non-deleted** half is *not* enforced by the foreign key and cannot be: an `Approval` is soft-deleted (§10.4 — the row stays and `IsDeleted` is set), so the key is satisfied by a deleted parent. It needs either a gated boolean probe over the unfiltered store, following the §14.6 pattern §9.7.2 rule 3 already uses for approval resolution, or a decision that commenting on a closed approval is permitted. **Open — neither has been chosen, and the non-deleted half is unenforced today (#214).**

**Deleted, not relocated: the versioning rules.** The withdrawn subsections gave `Tag`, `Reaction`, `Comment` and `BibleReference` a full versioning model — fork on modify of an `Approved` row, `IsLatestVersion` demotion, one latest and one published row per `GroupId`, and `GroupId` / `Version` / `IsLatestVersion` in the control-field list. The state precondition on modify went with them, and is replaced by shared rule 4 above. None of the four implements `IVersion` today; all four are `IApproval` only, and `EntityTypeVersioning` (§7.5.1) declares all four Single-Row. But the reason differs per entity, and the distinction matters:

- For `Tag`, `Reaction` and `Comment` the rules described **properties those types never had** — §5.1 and §5.2 describe `Tag` and `Reaction` as carrying `GroupId`/`Version`/`IsLatestVersion` which neither ever implemented, which is exactly the drift §7.5.1 rule 1 records.
- For `BibleReference` they described properties it **did** have and lost. It implemented `IVersion` with real `ContentItemGroupId`, `Version` and `IsLatestVersion` columns and their unique indexes; versioning was withdrawn **deliberately** in favour of the `USFM` natural key, for the §7.5.1 rule 4 reason. Its versioning rules are stale because of that decision, not because they were never true.
...
> **Entries 4–9 were withdrawn.** `ApprovalReviewOrchestration`, `ApprovalCommentOrchestration`, `TagOrchestration`, `ReactionOrchestration`, `CommentOrchestration` and `BibleReferenceOrchestration` were each planned here before the §12.1 rule was applied. Every one touches a single entity type, so none needs anything above its foundation — `Tag`, `Reaction`, `Comment` and `BibleReference` are additionally Single-Row (§7.5.1) so they never fork, while `ApprovalReview` and `ApprovalComment` are not approvable entities at all and never had a publication model (§12.3.1); their surviving rules moved to §12.3.1, and their versioning rules were deleted as describing properties those types never had.
>
