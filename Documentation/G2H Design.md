...
**It applies no role filter and no disabled filter, deliberately.** Every id sent to it came off a row somebody already stored, so the account is part of the record whatever has happened to it since; filtering by the review tier is exactly the mistake that left a departed reviewer with a blank name. It is a user-enumeration surface all the same, so the posture above governs it rather than being re-derived: the requesting tier and nobody else, an account id and a display name and nothing else. It is bounded as well — a batch of more than 200 ids is refused rather than truncated, since a silently shortened answer leaves the caller rendering blanks it cannot explain, and an uncapped resolver is how a name lookup becomes a directory dump. Ids naming no account come back absent rather than as an error, so one deleted account cannot blank a whole panel.
