...
2. **`IAccessClient` owns the decision logic** — approval counts, comment resolution, the rejection and zero-score blocks, bypass permission, self-approval permission. It **declares its own narrow read port** for the rows it needs (`ApprovalSetting`, `ApprovalReview`, `ApprovalComment`); `Glory2Him.Core` implements that port over its storage broker and injects it. The direction matters and is not stylistic: `Glory2Him.Core` holds a **project reference** to `G2H.Security.Client`, so a client that referenced `IStorageBroker` back would be a build cycle, not merely poor layering.
