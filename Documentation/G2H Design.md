...
**The AI seam has grown a second verb, and this service is not its caller.** §8.6.2.1's `AddAutomaticAIReviewerAssignmentAsync` is on `IAIReviewerAssignmentWorkflowService` since #534, called by `AIReviewerOrchestrationService`'s own subscriptions (§12.5 entry 4). This service goes on holding the seam for `ReturnStaleAIReviewerAssignmentToPendingAsync` alone, and its count of three is unchanged: a seam member this service never calls costs it no arm and no reference. The sentence above therefore says what **this service uses** rather than what the interface carries — an earlier form said the seam "carries one", which the second verb falsified here and in §16.7.1 at the same time.
