...
3. **The veto is scoped to writes — and the approval surface is one of them.** A block in scope stops the holder **voting on or deciding** the row's approval as well as writing the row itself: `Approval`, `ApprovalReview`, `ApprovalComment` and `ApprovalReviewRequest` have no role vocabulary of their own — there is no `Approval-Reviewers` — so their scope is derived from the attached entity, which for an association means both endpoints. A blocked holder is also not a reviewer *candidate* and cannot be invited (§7.9 rule 3, §16.7.4): inviting someone who cannot vote stalls the round for no reason.
...
**The per-caller answer accounts for the `ReadOnly` veto**, and gets it for free: `CanApprove` *is* the decision verdict, and the veto is evaluated inside it (§18.6 rule 2). Both halves matter. A blocked caller comes back `CanApprove = false` **and** `IsBypassAllowedForCurrentUser = false` — what a bypass waives are the §8.5 conditions, never the veto — and the refusal travels into `BlockReasons` as `BlockedByReadOnlyRole`, so the panel states the block rather than rendering "nothing is blocking this" beside a disabled control. The panel takes that answer verbatim rather than re-deriving it (§20.6.1); a surface that offered a decision the server then refuses would be reporting a bug that is not there. The message says the sanction applies and does **not** name which of the three scopes fired: that is the sanction's own detail, and no scope of it is appealable through this surface.
...
**A fifth operation, and it is a NAME RESOLVER rather than a fourth list.** `RetrieveReviewerDisplayNamesAsync(userIds)` answers what a set of account ids is called. It exists because `ApprovalReview` names its reviewer by `CreatedBy` — an account id — and nothing below the `Administrators`-only `/api/admin/users` could turn one into a name, so a `Publisher` who is not an administrator could render their own name and nobody else's. The candidates read does not close it: that read answers who is in scope for the round, so somebody who reviewed and then lost the role is absent from it entirely.

**One resolver, not a projection per read.** The panel needs names for reviewers, for invited people and for candidates; a display-name projection on the review read would have answered the first and left the next to invent its own, and three lookups are three chances to render one person under two names. Every surface instead hands over whatever ids it is holding, and every name is composed by one method — the same one the candidates read uses.

**That it is not keyed on a round is a separate choice, and a weaker one.** It does not follow from the paragraph above: all three surfaces the panel renders belong to the SAME round, so a round-scoped resolver would have served them just as well. Nor is round-scoping ruled out by the bug — a departed reviewer's id is still stamped on the review row, so a resolver scoped to the ids appearing on THIS approval's reviews, requests and candidates resolves them, which is exactly what the candidates read cannot do. What the unscoped form buys is one fewer read on the panel's hot path and a route that any future surface can call; what it costs is that the tier gate stands alone, so a holder of any review-tier role may resolve any account id rather than only the people a round they may already read involves. Composing the tier gate with an entity gate is the better posture and is recorded as open rather than settled here.

**It applies no role filter and no disabled filter, deliberately.** Every id sent to it came off a row somebody already stored, so the account is part of the record whatever has happened to it since; filtering by the review tier is exactly the mistake that left a departed reviewer with a blank name. It is a user-enumeration surface all the same, so the posture above governs it rather than being re-derived: the requesting tier and nobody else, an account id and a display name and nothing else. It is bounded as well — a batch naming more than 200 distinct accounts is refused rather than truncated, since a silently shortened answer leaves the caller rendering blanks it cannot explain. The count is taken after the ids are PARSED, because a GUID has several equal spellings and counting raw text would refuse a caller who asked about 200 people using 201 spellings of them. Ids naming no account come back absent rather than as an error, so one deleted account cannot blank a whole panel. The id echoed back is read off the resolved row, so it is always the canonical form whatever spelling was asked with, and a caller holding ids in another form normalises its own side to join on the answer.
...
| `GET` | `/api/Approvals/ReviewerDisplayNames?userIds=&userIds=` | What the given account ids are called — the §16.7.4 name resolver; requesting tier only. **Not keyed on a round**, which §16.7.4 records as open rather than settled: composing this tier gate with an entity gate is the better posture. No role or disabled filter, so a reviewer who has since left the tier still has a name. Ids naming nobody are absent, not an error; a batch naming more than 200 distinct accounts is a `400` rather than a truncated `200` — though the transport refuses a long batch first, since repeated ids exhaust IIS's 2048-character query string at roughly 45 and Kestrel's 8KB request line at roughly 180. A caller wanting many names pages. |
...
| Every name it renders | `GET api/Approvals/ReviewerDisplayNames` — the §16.7.4 resolver, asked once with the ids from the reviews, the requests and the candidates together |
