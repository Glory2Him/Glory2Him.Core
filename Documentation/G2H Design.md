...
>
> **The post-split inventory is therefore THREE arms each, and each service must carry all three** — twelve blocks, since an arm is four. `ApprovalOrchestrationService`: `Approval*`, `ApprovalReview*` (**landed by #518, stays**), `AIReviewerAssignment*`, and `ApprovalReviewRequest*` **removed** with the dependency. `ApprovalReviewerOrchestrationService`: `ApprovalReviewRequest*` (moved), `ApprovalComment*` (**landed by #518, moves**) and `IdentityUser*` (**landed by #518, moves**). Twelve downstream blocks apiece, at the ceiling and not over it. A reviewer should check the arms before the constructor: the arms are what the rule is actually measuring.
