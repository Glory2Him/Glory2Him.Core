...
| 5 | `ApprovalReviewerOrchestrationService` | **Built** (#521), except for the two retirements (§12.5.4). Who takes part in a round — reviewer candidates, invitations, withdrawal, and name resolution. Spans `ApprovalReviewRequest`, `ApprovalComment` and `IdentityUser`, and reads the `Approval` foundation for the read-triggered repair alone. §7.9 rules 6 and 8 still run on entry 3 until #522 turns them into subscriptions here; `ApprovalReviewersController` does not exist until #523, so `ApprovalsController` binds this service for the five reviewer routes meanwhile. **Four dependencies and four arms — an approved deviation, recorded in the register above.** |
...
19. **It holds three ARM-BEARING service references and three exception arms: `IApprovalWorkflowService`, `IApprovalReviewWorkflowService`, `IAIReviewerAssignmentWorkflowService`** — `Approval*`, `ApprovalReview*` and `AIReviewerAssignment*`, twelve downstream catch blocks, at the Florance ceiling and not over it. A **fourth** seam sits in the constructor and costs no arm: `IApprovalReviewRequestWorkflowService`, held knowingly temporarily for §7.9 rules 6 and 8 until #522 moves both retirements to `ApprovalReviewerOrchestrationService`'s substrate. It costs no arm for two reasons that both hold — the retirement verbs share `ApprovalReviewRequestService`'s own `TryCatch` with the caller-facing door, and each helper here absorbs or never reaches this service's catch chain. **The `ApprovalReview*` arm was landed ahead of this split, sequenced deliberately first (issue #518) so the split moves a working arm instead of writing it under the refactor**; `DismissStaleApprovalReviewAsync`'s validation-shaped refusals now report as `ApprovalOrchestrationDependencyValidationException` rather than falling to the catch-all. Two of those three carry a single verb — dismiss a stale review, return Berean to pending — and they stay together because they are the same act on two records: at the two sites where a round's verdicts stop describing its content (§9.7.4's `RequireReapprovalOnChange` branch and §8.6 HR-4's reset), the human reviews and the AI assignment are brought into line as one step. Splitting them would put half of one decision in another service. §16.7.1 states the counting rule these three are measured by.

#### 12.5.4 ApprovalReviewerOrchestrationService

> **The service is BUILT (#521); business rules 4 and 6 are not.** Nothing in this subsection is a new rule. Every rule it names is §7.9's, §16.7.2's or §16.7.4's, already written and already built inside `ApprovalOrchestrationService`; what this subsection settles is that they live on their own service, and what that costs.
>
> **What #521 landed:** responsibilities 1–5, the four exception arms, the read-triggered repair, and business rules 1, 2, 3, 5 and 7. **What is still designed and not built:** responsibility 6 and business rule 4 — the two retirements are still direct calls on `ApprovalOrchestrationService` and become subscriptions in #522 — and business rule 6, the controller split, which is #523. `ApprovalsController` therefore binds two orchestrations today, which is knowingly temporary and stated at the field that holds the second.
...
6. **The two retirements, reactively.** §7.9 rule 6 (an answered invitation retires itself) and rule 8 (a closed round retires what it never answered) move here with the rows they touch, and they arrive as subscriptions rather than as calls — see the business rules below.
...
4. **Both retirements are subscriptions, not calls, and this is the one behavioural change in the split.** Rule 6 binds `ApprovalReview-Added`; rule 8 binds `Approval-Modified`. Neither needs a new address, and neither needs a fact that says *why* the round moved — but rule 8 does need two things the direct call had for free, and an earlier draft of this rule assumed the broker supplied both. It supplies neither.
   ...
   **(ii) The gather must exclude whoever answered, so the two retirements stop depending on their order.** The broker's filter is `IsDeleted == false`, which today means "what the round is still waiting on" **only because rule 6 has already soft-deleted the answerer's row** — an ordering the current code makes explicit and this design would silently drop. Two independent subscriptions on `ApprovalReview-Added` have no stated order, and the losing order is the common case: the last reviewer's vote completes the round, the re-test's `ModifyApprovalAsync` publishes `-Modified` synchronously, and rule 8's sweep takes the answerer's own invitation and stamps it `"Retired: the approval round closed before this review was cast"` — false about the one person who did cast one. §7.9 rule 8 makes `DeletionReason` the reader's only way to tell a close-retirement from a withdrawal, so this degrades the distinction the rule exists to protect.
