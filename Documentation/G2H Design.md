...
| 13 | `AIReviewersController` | Exposes `GET`/`POST`/`DELETE api/Approvals/{entityType}/{entityId}/AIReviewer` (§8.6.2). **Built** (#474), bound to `IAIReviewerOrchestrationService` alone, and absent from this table until now — the same drift §12.5's orchestration table carried for the service behind it.
