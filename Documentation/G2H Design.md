...
1. **Contribution gate.** Blocked by the global `ReadOnly`, **or** by `%EntityAType%-ReadOnly`, **or** by `%EntityBType%-ReadOnly`. **The `OR` is load-bearing.** Under an `AND`, a user holding `Tag-ReadOnly` alongside `BibleReference-Reviewers` could pair a tag with an entity type they are not banned from and land it on a public scripture page — exactly what `Tag-ReadOnly` exists to prevent. A block on one end blocks the association.
...
**Content-type-scoped roles.** `ContentItem` has a further granularity: `%EntityType%-%ContentType%-Reviewers` and `-Publishers`, so a reviewer can be trusted with stories but not testimonies.

```text
ContentItem-Story-Reviewers,       ContentItem-Story-Publishers,
ContentItem-Series-Reviewers,      ContentItem-Series-Publishers,
ContentItem-Testimony-Reviewers,   ContentItem-Testimony-Publishers
```
...
**`ReadOnly` is the exception and stays singular**, wherever it appears — the global `ReadOnly` and the scoped `%EntityType%-ReadOnly` alike. It does not name a group of people, it names the *state its holder is in*, and it has no sensible plural. That is a decision, written here so it is not later read as an oversight and "corrected" into line with the other two.
