...
2. **It is what will make uniqueness a database guarantee.** Two `AllVersions` rows for the same group differing only in `KeyId` mean the same thing; over the raw columns they are distinct rows, and the effective id collapses them. This matters because foundation services are reachable through public event addresses and cannot assume an orchestration's retrieve-or-add ran first. The column exists and is indexed; the unique index over it (`UX_Associations_Pair`) is separate schema work and is **not yet in place**, so today nothing stops a duplicate pair being inserted.
...
**Current state — an association cannot yet become public.** Rules 3 and 5 both refer to public rows, and there is no way to produce one. The foundation refuses a caller-supplied `IsPublished`, `PublishDate` or non-`Draft`/`Submitted` `ApprovalStatus` on add, and pins all three against storage on modify; no orchestration, approve operation or broker writes them. That is deliberate — it closes a hole where any authenticated caller could insert an already-approved, already-published row — but it means every association read currently requires ownership or a review role, and the public clause on both read paths is unreachable for service-written data. It becomes reachable when the association approval operation lands; until then, do not read the public clause as a live path.
