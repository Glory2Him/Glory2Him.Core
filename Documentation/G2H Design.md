...
**Settings and gating — two booleans, and the second is a child of the first. BUILT.**

1. **`IsAIReviewerOffered`** is the feature switch. With it on, Berean is offered in the reviewer-request UI and, once asked, answers on the round in `ApprovalComment`s. **Commenting is not separately gated**, and an earlier draft that gave it its own switch was wrong: an AI reviewer you may assign is one that may answer, and a setting that offered Berean while silencing it would describe a reviewer who can be asked and can never reply. With it off nothing is offered and nothing is performed.
2. **`IsAIAllowedToVote`** requires the first, and governs one thing: whether Berean may **additionally** cast an `ApprovalReview`.

Both resolve through the normal §8.4 tiering (`(EntityType, ContentType, IsPersonal)`), so AI participation can be enabled per content type exactly like every other approval policy, and both fail closed when no row resolves.
...
| 3 | `ApprovalsController` | Exposes endpoints for approval submission and status retrieval — the verdict, the decision and the reset. Bound to `IApprovalOrchestrationService` for those three, and — **knowingly temporarily, until #523 splits it** — to `IApprovalReviewerOrchestrationService` for the five reviewer routes entry 3a will take. |
| 3a | `ApprovalReviewersController` | **Designed, not built** (§12.5.4 business rule 6) — #523. Until it exists, entry 3 binds `IApprovalReviewerOrchestrationService` alongside its own service and hosts these five routes, which is why entry 3 above no longer says "bound to `IApprovalOrchestrationService` alone". Exposes the reviewer-coordination endpoints — candidates, display names and the three review-request verbs — bound to `IApprovalReviewerOrchestrationService` alone. It carries `[Route("api/Approvals")]` explicitly rather than by convention, so the URLs do not move and no client changes. Numbered `3a` because it is entry 3 split in two, not a new resource, and named in the PLURAL like every sibling exposer — `ApprovalsController`, `ApprovalReviewsController`, `ApprovalCommentsController`, `AIReviewersController`. Entry 3 read `ApprovalController` here while the class on disk has always been `ApprovalsController`; that is corrected above rather than carried into a second name. |
