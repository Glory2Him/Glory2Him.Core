...
- **`RequestApprovalReviewAsync(entityType, entityId, requestedUserId)`** — validates the §7.9 rules (target eligibility, owner exclusion, the `Submitted` window) and lands the request, or returns the existing state on a duplicate (rule 4).
...
- **`RetrieveReviewerCandidatesAsync(entityType, entityId)`** — who *can* be asked: users satisfying the review tier for the entity, minus the entity's owner, minus anyone already holding an active review or an active request on this approval.
...
| `POST` | `/api/Approvals/{entityType}/{entityId}/ReviewRequests?requestedUserId=` | Invite an eligible user to review (§7.9); idempotent on duplicates. |
| `DELETE` | `/api/ApprovalReviewRequests/{id}?deletionReason=` | Withdraw a pending review request (soft delete, requesting tier). |
