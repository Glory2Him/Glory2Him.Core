...
1. **Three service dependencies, three exception arms, ELEVEN downstream blocks — not twelve: `IApprovalReviewRequestService`, `IApprovalCommentService`, `IIdentityUserService`** — plus `IAccessBroker` and the envelope, event, integrity and logging brokers. `IApprovalReviewRequestService`'s workflow seam (`IApprovalReviewRequestWorkflowService`) is a second door onto the same class **and shares its `TryCatch`** — `RetireApprovalReviewRequestAsync` wraps both retirement verbs in the same one the caller-facing operations use — so both doors throw `ApprovalReviewRequest*` and cost one arm between them. That shared family, not the shared instance, is what keeps the count at three; §16.7.1 states the rule and why the instance is the wrong thing to count. `ApprovalReviewRequest*` and `ApprovalComment*` are four-block arms; `IdentityUser*` is three, per finding 1 — four + four + three = eleven, not the twelve a same-sized-arm assumption would predict.

   **Two of the three arms already exist on the service these operations are moving off, landed ahead of this split by issue #518 for the same reason `ApprovalReview*` was (§12.5.3 business rule 19)**: `ApprovalComment*` and `IdentityUser*` now have handlers there, so their validation-shaped refusals report as `ApprovalOrchestrationDependencyValidationException` rather than the 424 they used to. This subsection's work is to move those arms with the operations that raise them, not to write them.

2. **It takes no `IApprovalWorkflowService`, and that is what keeps it at three.** Every operation is keyed on an entity, and every one of them needs the round behind that key. Resolving it through the `Approval` foundation would add a fourth arm to read a row `IAccessBroker.RetrieveApprovalReviewerScopeByIdAsync` was already going to read. **A by-entity overload is added instead** — `RetrieveApprovalReviewerScopeByEntityAsync(entityType, entityId, cancellationToken)` — which resolves the approval from the entity key and returns the same `ApprovalReviewerScope`, whose `ApprovalId` is then what the request reads and writes are keyed on. It answers `null` where no round exists, exactly as the by-id form answers `null` for an approval that is not there, and the caller reports that as not-found. Resolving an `EntityType` to a row is the one thing this broker exists for (§12.3.1), so the overload adds no capability — only a second entry point to one it has.

3. **`IApprovalCommentService` is a caller-facing read and must stay one.** The display-name resolver names the authors of the round's live comments, and §16.7.4 rules that they "arrive through `IApprovalCommentService`, so §14.7 posture D's own visibility filter decides which authors a given caller can name". Reading them off the gathering broker instead would name people whose words the reader cannot see. This is the reason the count is three rather than two, and it is not tradeable.
...
4. **Both retirements are subscriptions, not calls, and this is the one behavioural change in the split.** Rule 6 binds `ApprovalReview-Added`; rule 8 binds `Approval-Modified`. Neither needs a new address, and neither needs a fact that says *why* the round moved — but rule 8 does need two things the direct call had for free, and an earlier draft of this rule assumed the broker supplied both. It supplies neither.
   ...
   **`FindRetirableApprovalReviewRequestIdsAsync` therefore excludes any `RequestedUserId` holding a review that still STANDS on the round** — `IsDeleted == false && Verdict != Dismissed`, which is `ApprovalReviewerScope.ActiveReviewerUserIds`'s own predicate. It governs here for the same reason it governs invitability: it is the set the already-answered short-circuit (`ApprovalOrchestrationService.ReviewRequests.cs:159`) gates on, so the exclusion and invitability stay one definition rather than two that can drift.
...
| 3 | `ApprovalsController` | Exposes endpoints for approval submission and status retrieval — the verdict, the decision and the reset. Bound to `IApprovalOrchestrationService` alone. |
| 3a | `ApprovalReviewersController` | **Designed, not built** (§12.5.4 business rule 6). Exposes the reviewer-coordination endpoints — candidates, display names and the three review-request verbs — bound to `IApprovalReviewerOrchestrationService` alone. It carries `[Route("api/Approvals")]` explicitly rather than by convention, so the URLs do not move and no client changes. Numbered `3a` because it is entry 3 split in two, not a new resource, and named in the PLURAL like every sibling exposer — `ApprovalsController`, `ApprovalReviewsController`, `ApprovalCommentsController`, `AIReviewersController`. Entry 3 read `ApprovalController` here while the class on disk has always been `ApprovalsController`; that is corrected above rather than carried into a second name. |
...
>
> **The post-split inventory is therefore THREE arms each, and each service must carry all three — but not every arm is four blocks, and the total is not twelve apiece.** `ApprovalOrchestrationService`: `Approval*`, `ApprovalReview*` (**landed by #518, stays**), `AIReviewerAssignment*`, and `ApprovalReviewRequest*` **removed** with the dependency — three FOUR-block arms, **twelve blocks**. `ApprovalReviewerOrchestrationService`: `ApprovalReviewRequest*` (moved), `ApprovalComment*` (**landed by #518, moves**) and `IdentityUser*` (**landed by #518, moves**) — two four-block arms and `IdentityUser*`'s own THREE-block arm (finding 1: no `IdentityUserDependencyValidationException` exists or should), **eleven blocks**. Twenty-three downstream blocks between the two services today, on the one class that carries all six families until the split moves three of them. A reviewer should check the arms before the constructor: the arms are what the rule is actually measuring.
