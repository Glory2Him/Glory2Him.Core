...
> **`.claude/agents/architect.md` states this the other way — "Brokers remain off limits to an orchestration entirely" — and that sentence is wrong about this solution rather than describing an unmet goal.** It was written as the tail of the same-kind rule, whose real subject is that an orchestration must not mix processing services with foundation services. Read literally it forbids `ILoggingBroker`, which would leave an orchestration unable to log, and it forbids `IAccessBroker`, which would put §8.4's most-specific-wins back inside every service that asks a policy question — the exact duplication §8.6.1 rule 4 exists to prevent. The agent file is the owner of the same-kind rule (per the repository `CLAUDE.md`), so **correcting that one clause is part of this change**, not a separate cleanup: the sentence should bar an orchestration from a **storage** broker, which is the boundary actually being protected — an orchestration reaching storage directly would skip every layer beneath it at once.
...
4. **Both retirements are subscriptions, not calls, and this is the one behavioural change in the split.** Rule 6 binds `ApprovalReview-Added`; rule 8 binds `Approval-Modified`. Neither needs a new address and neither needs the fact to say *why* the round moved, because `IAccessBroker` already answers the whole condition: `FindRetirableApprovalReviewRequestIdsAsync` returns the ids to retire and returns an empty list on a round that is still open, so a `-Modified` that closed nothing costs one gather and writes nothing.
...
| 3 | `ApprovalController` | Exposes endpoints for approval submission and status retrieval — the verdict, the decision and the reset. Bound to `IApprovalOrchestrationService` alone. |
| 3a | `ApprovalReviewerController` | **Designed, not built** (§12.5.4 business rule 6). Exposes the reviewer-coordination endpoints — candidates, display names and the three review-request verbs — bound to `IApprovalReviewerOrchestrationService` alone. It carries `[Route("api/Approvals")]` explicitly rather than by convention, so the URLs do not move and no client changes. Numbered `3a` because it is entry 3 split in two, not a new resource. |
...
>
> **The post-split inventory is therefore four arms each, and each service must carry all four.** `ApprovalOrchestrationService`: `Approval*`, `ApprovalReview*` (**to add**), `AIReviewerAssignment*`, and `ApprovalReviewRequest*` **removed** with the dependency. `ApprovalReviewerOrchestrationService`: `ApprovalReviewRequest*` (moved), `ApprovalComment*` (**to add**) and `IdentityUser*` (**to add**). Twelve downstream blocks apiece, at the ceiling and not over it. A reviewer should check the arms before the constructor: the arms are what the rule is actually measuring.
...
> **The last five rows move to `ApprovalReviewerController` and keep their URLs** (§12.5.4 business rule 6, §12.6 entry 3a). They are listed here rather than in a table of their own because a reader looking up an approval URL should find it in one place; which controller class answers it is §12.6's business, not a caller's.
