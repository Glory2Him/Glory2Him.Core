...
2b. **A review may only be written while its `Approval` is `Submitted`** — this is the window, and it is not yet enforced. Once the `Approval` reaches `Approved` or `Rejected` the round is over, and a verdict changed afterwards would not re-run the workflow: an entity could sit `Approved` with a standing rejection against it and nothing would notice. The check needs the parent `Approval`'s status, which is another entity's row, so it belongs to `IAccessClient` (§8.6.1) and is recorded there as an open gap. Rules 2 and 2a are row-local and are enforced today.
...
**Interim gap — the review window is not enforced.** §7.7 rule 2b says an `ApprovalReview` may only be written while its `Approval` is `Submitted`. Nothing checks it: `ApprovalReviewService` holds the review row and never reads `Approval`, and reaching for it would make a single-entity service cross-entity. So a verdict can be changed after the round closed, and because nothing re-runs the workflow on a review write, an entity can sit `Approved` with a standing rejection against it. `Approval` is already in the read port above for HR-1, so this costs one extra field on the same read.
