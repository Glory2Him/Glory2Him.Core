...
9. **An amendment of a terminal row is refused.** §7.5.1 rule 3: `Single-Row` means the row that is edited **is** the published row, so there is nothing to fork into and an amendment of a terminal (`Approved` or `Rejected`) row is refused instead, the only route back being an `Admin` override to `Submitted` that dismisses active reviews (§8.8 regardless-rule 1). §3.4 rule 7 generalises it: a terminal row's content is immutable.
    ...
    **On those two the rule is written against the amendment, not against the status**, and the distinction is load-bearing rather than pedantic. The fork does not leave the terminal row untouched: it **demotes** it, flipping `IsLatestVersion` to `false` through that same general modify. A refusal written against the stored status alone would therefore refuse the demotion and break the very fork it exists to redirect amendments into. So `ContentItemService` and `LinkService` compare the caller-editable content against storage — `Title`/`Author`/`Content`, and `Name`/`Url`/`LinkType` — and refuse only when it differs. The five `Single-Row` entities never fork, so nothing legitimately writes a terminal row of theirs and the blunt refusal is both safe and stricter.

    Comparing content rather than trusting the status pin is the whole point: the pin's condition is guarded by `inputStatus != storageStatus`, which is exactly the guard a caller echoing the stored status back walks through.
