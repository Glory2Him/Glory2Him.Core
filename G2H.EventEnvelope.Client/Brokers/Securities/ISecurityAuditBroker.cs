...
namespace G2H.EventEnvelope.Client.Brokers.Securities
{
    ...
    internal interface ISecurityAuditBroker
    {
        ...
        /// </summary>
        ValueTask<T> ApplyAddAuditValuesAsync<T>(T entity, EventSecurityContext securityContext);

        /// <summary>
        /// Applies modify audit values as the actor carried on an event envelope's
        /// <c>SecurityContext</c>.
        /// </summary>
        ValueTask<T> ApplyModifyAuditValuesAsync<T>(T entity, EventSecurityContext securityContext);

        /// <summary>
        /// Applies remove audit values as the actor carried on an event envelope's
        /// <c>SecurityContext</c>.
        /// </summary>
        ValueTask<T> ApplyRemoveAuditValuesAsync<T>(T entity, EventSecurityContext securityContext);
        ...
        /// </summary>
        ValueTask<string> GetUserIdAsync(EventSecurityContext securityContext);
    }
}
