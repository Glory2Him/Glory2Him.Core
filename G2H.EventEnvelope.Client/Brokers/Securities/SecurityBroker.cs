...
using G2H.EventEnvelope.Client.Models.Securities;
...
namespace G2H.EventEnvelope.Client.Brokers.Securities
{
    ...
    internal class SecurityBroker : ISecurityBroker
    {
        ...
        /// </summary>
        /// <returns>An <see cref="EventUser"/> object containing user details.</returns>
        public async ValueTask<EventUser> GetCurrentUserAsync()
        {
        ...
        /// carrying on an <see cref="Core.Models.Events.EventEnvelope{T}"/>, so
        /// event handlers can evaluate the original actor without depending on the HTTP context.
        /// </summary>
        /// <returns>The current caller's <see cref="EventSecurityContext"/>.</returns>
        public async ValueTask<EventSecurityContext> GetCurrentSecurityContextAsync()
        {
            ...
            EventUser user = await GetCurrentUserAsync();
