...
namespace Glory2Him.Core.Tests.Unit.Services.Foundations.BibleReferences
{
    ...
    {
        /// <summary>
        /// §10.19. <c>BibleReference-Submitted</c> is a REQUIRED delivery: it reaches
        /// <c>ApprovalOrchestrationService.OnBibleReferenceSubmittedAsync</c>, which moves the
        /// Bible reference's approval to Submitted and re-evaluates the round. The substrate
        ...
        public async Task ShouldLogCriticalWhenTheSubmittedFactDeliveryFailsAsync()
        {
            ...
            var failedPublishResult = new EventPublishResult<BibleReference>
            {
                EventId = publishedEventId,
                Deliveries = new List<EventDelivery<BibleReference>>
                {
                    new EventDelivery<BibleReference>
                    {
        ...
        public async Task ShouldNotLogWhenEverySubmittedDeliverySucceedsAsync()
        {
            ...
            var succeededPublishResult = new EventPublishResult<BibleReference>
            {
