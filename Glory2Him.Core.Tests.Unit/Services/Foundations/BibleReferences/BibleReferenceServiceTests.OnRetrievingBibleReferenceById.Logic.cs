...
namespace Glory2Him.Core.Tests.Unit.Services.Foundations.BibleReferences
{
    ...
    {
        [Fact]
        public async Task ShouldReplyWithBibleReferenceOnRetrievingBibleReferenceByIdEventAsync()
        {
            // given
            ...
            {
                Content = new BibleReference { Id = randomBibleReference.Id }
            };
            ...
            EventEnvelope<BibleReference>? actualReplyEnvelope =
                await this.bibleReferenceService.OnRetrievingBibleReferenceByIdAsync(
                    requestEnvelope,
                    TestContext.Current.CancellationToken);
