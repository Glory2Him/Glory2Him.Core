...
namespace Glory2Him.Core.Tests.Unit.Services.Foundations.ApprovalSettings
{
    ...
    {
        [Fact]
        public async Task ShouldHardRemoveApprovalSettingByIdAndReplyOnHardRemovingApprovalSettingByIdEventAsync()
        {
            // given
            ApprovalSetting storageApprovalSetting = CreateRandomApprovalSetting();
            ApprovalSetting deletedApprovalSetting = storageApprovalSetting.DeepClone();
            ApprovalSetting expectedApprovalSetting = deletedApprovalSetting.DeepClone();
            ...
            this.storageBrokerMock.Setup(broker =>
                broker.DeleteApprovalSettingAsync(storageApprovalSetting, It.IsAny<CancellationToken>()))
                    .ReturnsAsync(deletedApprovalSetting);
            ...
            EventEnvelope<ApprovalSetting>? actualReplyEnvelope =
                await this.approvalSettingService.OnHardRemovingApprovalSettingByIdAsync(
                    requestEnvelope,
                    TestContext.Current.CancellationToken);
        ...
        [Fact]
        public async Task ShouldSkipHardRemoveAndReplyNullWhenHardRemovingApprovalSettingByIdEventAlreadyProcessedAsync()
        {
            // given
            var requestEnvelope = new EventEnvelope<ApprovalSetting>
            {
