...
namespace Glory2Him.Core.Tests.Unit.Services.Foundations.ContentTypes
{
    ...
    {
        [Fact]
        public async Task ShouldHardRemoveContentTypeByIdAndReplyOnHardRemovingContentTypeByIdEventAsync()
        {
            // given
            ContentType storageContentType = CreateRandomContentType();
            ContentType deletedContentType = storageContentType.DeepClone();
            ContentType expectedContentType = deletedContentType.DeepClone();
            ...
            this.storageBrokerMock.Setup(broker =>
                broker.DeleteContentTypeAsync(storageContentType, It.IsAny<CancellationToken>()))
                    .ReturnsAsync(deletedContentType);
            ...
            EventEnvelope<ContentType>? actualReplyEnvelope =
                await this.contentTypeService.OnHardRemovingContentTypeByIdAsync(
                    requestEnvelope,
                    TestContext.Current.CancellationToken);
        ...
        [Fact]
        public async Task ShouldSkipHardRemoveAndReplyNullWhenHardRemovingContentTypeByIdEventAlreadyProcessedAsync()
        {
            // given
            var requestEnvelope = new EventEnvelope<ContentType>
            {
