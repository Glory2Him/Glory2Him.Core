...
namespace Glory2Him.Core.Tests.Unit.Services.Foundations.Comments
{
    ...
    {
        [Fact]
        public async Task ShouldRemoveCommentByIdAndReplyOnRemovingCommentByIdEventAsync()
        {
            // given
            ...
            Comment removedComment = auditedComment.DeepClone();
            Comment expectedComment = removedComment.DeepClone();
            ...
            this.securityAuditBrokerMock.Setup(broker =>
                broker.ApplyRemoveAuditValuesAsync(storageComment, It.IsAny<SecurityContext>(), randomDeletionReason))
                    .ReturnsAsync(auditedComment);

            this.storageBrokerMock.Setup(broker =>
                broker.UpdateCommentAsync(auditedComment, It.IsAny<CancellationToken>()))
                    .ReturnsAsync(removedComment);
            ...
            EventEnvelope<Comment>? actualReplyEnvelope =
                await this.commentService.OnRemovingCommentByIdAsync(
                    requestEnvelope,
                    TestContext.Current.CancellationToken);
        ...
        [Fact]
        public async Task ShouldSkipRemoveAndReplyNullWhenRemovingCommentByIdEventAlreadyProcessedAsync()
        {
            // given
            var requestEnvelope = new EventEnvelope<Comment>
            {
        ...
        [Fact]
        public async Task ShouldReplyWithExistingCommentOnRemovingCommentByIdEventWhenAlreadyDeletedAsync()
        {
            // given
            Comment alreadyDeletedComment = CreateRandomComment();
            alreadyDeletedComment.IsDeleted = true;
            Comment expectedComment = alreadyDeletedComment.DeepClone();

            var requestEnvelope = new EventEnvelope<Comment>
            {
                SecurityContext = CreateAuthenticatedSecurityContext(),
                Content = new Comment { Id = alreadyDeletedComment.Id },
                Metadata = new EventMetadata { EventId = Guid.NewGuid() }
            };
