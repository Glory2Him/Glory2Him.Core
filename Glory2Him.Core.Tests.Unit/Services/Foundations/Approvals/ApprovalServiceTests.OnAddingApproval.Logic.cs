...
namespace Glory2Him.Core.Tests.Unit.Services.Foundations.Approvals
{
    ...
    {
        [Fact]
        public async Task ShouldAddApprovalAndReplyOnAddingApprovalEventAsync()
        {
            // given
            ...
            EventEnvelope<Approval>? actualReplyEnvelope =
                await this.approvalService.OnAddingApprovalAsync(
                    requestEnvelope,
                    TestContext.Current.CancellationToken);
        ...
        [Fact]
        public async Task ShouldSkipAddAndReplyNullWhenAddingApprovalEventAlreadyProcessedAsync()
        {
            // given
            var requestEnvelope = new EventEnvelope<Approval>
            {
