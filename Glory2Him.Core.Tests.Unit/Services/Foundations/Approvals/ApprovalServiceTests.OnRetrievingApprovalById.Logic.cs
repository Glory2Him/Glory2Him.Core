...
namespace Glory2Him.Core.Tests.Unit.Services.Foundations.Approvals
{
    ...
    {
        [Fact]
        public async Task ShouldReplyWithApprovalOnRetrievingApprovalByIdEventAsync()
        {
            // given
            ...
            EventEnvelope<Approval>? actualReplyEnvelope =
                await this.approvalService.OnRetrievingApprovalByIdAsync(
                    requestEnvelope,
                    TestContext.Current.CancellationToken);
            ...
            this.eventEnvelopeFactoryMock.Verify(factory =>
                factory.CreateNextAsync(requestEnvelope, storageApproval),
                Times.Once);
