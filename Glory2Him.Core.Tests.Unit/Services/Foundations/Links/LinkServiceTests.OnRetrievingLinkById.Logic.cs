...
namespace Glory2Him.Core.Tests.Unit.Services.Foundations.Links
{
    ...
    {
        [Fact]
        public async Task ShouldReplyWithLinkOnRetrievingLinkByIdEventAsync()
        {
            // given
            ...
            EventEnvelope<Link>? actualReplyEnvelope =
                await this.linkService.OnRetrievingLinkByIdAsync(
                    requestEnvelope,
                    TestContext.Current.CancellationToken);
            ...
            this.eventEnvelopeFactoryMock.Verify(factory =>
                factory.CreateNextAsync(requestEnvelope, storageLink),
                Times.Once);
