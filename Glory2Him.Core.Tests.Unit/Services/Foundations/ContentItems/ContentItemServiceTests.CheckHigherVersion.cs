...
namespace Glory2Him.Core.Tests.Unit.Services.Foundations.ContentItems
{
    ...
    /// catalogue in <c>ContentItemNarrowReadTests</c>.</para>
    ///
    ...
    public partial class ContentItemServiceTests
    {
        ...
        public async Task ShouldReportAHigherVersionExistsWhenStorageDoesAsync()
        {
            ...
            this.storageBrokerMock.Setup(broker =>
                broker.ExistsHigherLiveContentItemVersionInGroupAsync(
                    inputGroupId, inputVersion, It.IsAny<CancellationToken>()))
                        .ReturnsAsync(true);

            // when
            bool actualResult =
                await this.contentItemService.CheckHigherContentItemVersionExistsAsync(
                    inputGroupId,
                    inputVersion,
        ...
        public async Task ShouldThrowValidationExceptionOnCheckHigherVersionIfCallerIsNotAuthenticatedAsync()
        {
            ...
            ValueTask<bool> checkTask =
                this.contentItemService.CheckHigherContentItemVersionExistsAsync(
                    Guid.NewGuid(),
                    version: 1,
                    TestContext.Current.CancellationToken);

            ContentItemValidationException actualException =
                await Assert.ThrowsAsync<ContentItemValidationException>(checkTask.AsTask);

            // then
            actualException.InnerException.Should().BeOfType<UnauthorizedContentItemException>();
