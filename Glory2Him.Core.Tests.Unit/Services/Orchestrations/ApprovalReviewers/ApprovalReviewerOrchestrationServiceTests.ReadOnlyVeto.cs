...
{
    /// <summary>
    /// How the <c>ReadOnly</c> veto reaches the reviewer SURFACE (design §18.6 rule 2, §16.7.4,
    /// §7.9). An approval has no role vocabulary of its own — its scope is derived from the attached
    /// entity — so a block in scope stops the holder voting on it, not merely writing the content.
    ///
    /// <para>The candidate list and the invitation have to subtract the blocked explicitly,
    /// because inviting somebody who cannot vote stalls the round for no reason. The verdict's own
    /// half of the veto gets its answer for free and is asserted where the verdict lives
    /// (ApprovalOrchestrationServiceTests.ReadOnlyVeto).</para>
    /// </summary>
